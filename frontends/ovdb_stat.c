...
static void
display_heading(const char *str)
{
    if (html)
        printf("<h2>%s</h2>\n", str);
    else
...
static void
display_data(void *p, struct datatab *tab)
{
    int i;
...
static void
start_table(const char *label, struct datatab *tab)
{
    int i;
    if (html) {
        printf("<h2>%s</h2>\n", label);
...
static void
display_row(void *p, struct datatab *tab)
{
    int i;
...
static struct datatab LOCK_tab[] = {
 {INT32,  F(st_id),            -1, -1, "Last allocated locker ID"},
 ...
 {DIFF32, F(st_nrequests), F(st_nreleases), F(st_ndeadlocks),
                                       "Outstanding locks"},
...
static struct datatab LOG_tab[] = {
 ...
 {BYTES, F(st_wc_bytes), F(st_wc_mbytes), -1,
 ...
 {INT32, F(st_region_nowait),     -1, -1,
                                      "Region locks granted without waiting"},
 {INT32, F(st_region_wait),       -1, -1,
...
static struct datatab MEM_tab[] = {
 ...
 {INT32, F(st_page_trickle),  -1, -1,
                           "Dirty buffers written by trickle-sync thread"},
...
static int
txn_compare(const void *a, const void *b)
{
    if (((const DB_TXN_ACTIVE *) a)->txnid
        > ((const DB_TXN_ACTIVE *) b)->txnid)
        return 1;
    if (((const DB_TXN_ACTIVE *) a)->txnid
        < ((const DB_TXN_ACTIVE *) b)->txnid)
        return -1;
...
display_txn(void)
{
    ...
    if (sp->st_nactive) {
        qsort(sp->st_txnarray, sp->st_nactive, sizeof(sp->st_txnarray[0]),
              txn_compare);
...
static int
parse_artrange(char *str, ARTNUM *start, ARTNUM *stop)
{
    char *c;
    int i;
...
main(int argc, char *argv[])
{
    ...
    int disp_lock = 0, disp_log = 0, disp_mem = 0, disp_mem_all = 0,
        disp_txn = 0, disp_ver = 0;
    int needng = 0, o;
        ...
        for (o = optind; o < argc; o++) {
            ...
            if (getgs) {
                if (ovdb_groupstats(argv[o], &lowi, &highi, &count, &flag)) {
                    if (html)
                        printf("<td>%d<td>%d<td>%d<td>%c", lowi, highi, count,
                               flag);
                    else
                        printf("%s: groupstats: low: %d, high: %d, count: %d, "
                               "flag: %c\n",
