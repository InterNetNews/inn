 ...
 *              record is read.
 * 2000-10-05 : artnum member of struct datakey changed from ARTNUM to
 *              u_int32_t.  OS's where sizeof(long)==8 will have to rebuild
 *              their databases after this update.
...
bool
ovdb_groupstats(const char *group UNUSED, int *lo UNUSED, int *hi UNUSED,
...
bool
ovdb_groupadd(const char *group UNUSED, ARTNUM lo UNUSED, ARTNUM hi UNUSED,
              char *flag UNUSED)
{
    return false;
}
...
bool
ovdb_groupdel(const char *group UNUSED)
{
    return false;
}

bool
ovdb_add(const char *group UNUSED, ARTNUM artnum UNUSED, TOKEN token UNUSED,
         char *data UNUSED, int len UNUSED, time_t arrived UNUSED,
...
bool
ovdb_cancel(const char *group UNUSED, ARTNUM artnum UNUSED)
{
    return false;
}

void *
ovdb_opensearch(const char *group UNUSED, int low UNUSED, int high UNUSED)
{
    return NULL;
}

bool
ovdb_search(void *handle UNUSED, ARTNUM *artnum UNUSED, char **data UNUSED,
...
void
ovdb_closesearch(void *handle UNUSED)
{
}

bool
ovdb_getartinfo(const char *group UNUSED, ARTNUM artnum UNUSED,
                TOKEN *token UNUSED)
{
    return false;
}
...
bool
ovdb_expiregroup(const char *group UNUSED, int *lo UNUSED,
...
bool
ovdb_ctl(OVCTLTYPE type UNUSED, void *val UNUSED)
{
    return false;
}
...
static bool
conf_bool_val(char *str, bool *value)
{
...
static bool
conf_long_val(char *str, long *value)
{
    long v;
...
static void
OVDBerror(const DB_ENV *dbenv UNUSED, const char *db_errpfx UNUSED,
          const char *buffer)
{
    warn("OVDB: %s", buffer);
}
...
open_db_file(int which)
{
    ...
    ret = (dbs[which])
              ->open(dbs[which], tid, name, NULL, DB_BTREE, _db_flags, 0666);
    if (ret == 0)
...
static int
groupid_new(group_id_t *gno, DB_TXN *tid)
{
    DBT key, val;
    int ret, n;
    ...
    if (val.size < sizeof(group_id_t) || val.size % sizeof(group_id_t) != 0) {
...
/* must be used in a transaction */
static int
groupid_free(group_id_t gno, DB_TXN *tid)
{
    DBT key, val;
    int ret, n, i;
    group_id_t *freelist;
...
/* Must be called outside of a transaction because it makes its own
   transactions */
static int
delete_all_records(int whichdb, group_id_t gno)
{
    DB *db;
    DBC *dbcursor;
    DBT key, val;
    ...
    while (1) {
        ...
        for (count = 0; count < DELETE_TXN_SIZE; count++) {
            ...
            if (key.size == sizeof dk && memcmp(key.data, &gno, sizeof gno)) {
                break;
            }
...
static int
count_records(struct groupinfo *gi)
{
    int ret;
    DB *db;
    DBC *cursor;
    ...
    while (ret == 0 && key.size == sizeof(dk)
           && dk.groupnum == gi->current_gid) {
...
static int lockfd = -1;
bool
ovdb_getlock(int mode)
{
    if (lockfd == -1) {
        char *lockfn = concatpath(innconf->pathrun, OVDB_LOCKFN);
        lockfd =
            open(lockfn, mode == OVDB_LOCK_NORMAL ? O_RDWR : O_CREAT | O_RDWR,
                 0660);
        if (lockfd == -1) {
            if (errno == ENOENT)
...
bool
ovdb_check_pidfile(const char *file)
{
    int f, pid;
    char buf[SMBUF];
...
int
ovdb_open_berkeleydb(int mode, int flags)
{
    int ret;
    u_int32_t ai_flags =
    ...
    if ((flags & (OVDB_UPGRADE | OVDB_RECOVER))
        == (OVDB_UPGRADE | OVDB_RECOVER))
        ai_flags |= DB_PRIVATE;
...
#    endif
    ...
    /* Hmm...  Berkeley DB needs something like a 'struct iovec' so that we
       don't have to make a new buffer and copy everything in to it. */
...
ovdb_opensearch(const char *group, int low, int high)
{
    ...
    if (nsearches == maxsearches) {
        maxsearches += 50;
        searches =
            xrealloc(searches, sizeof(struct ovdbsearch *) * maxsearches);
    }
...
bool
ovdb_search(void *handle, ARTNUM *artnum, char **data, int *len, TOKEN *token,
            time_t *arrived)
{
    ...
    if ((len || data) && val.size - sizeof(struct ovdata) > (size_t) INT_MAX) {
...
ovdb_expiregroup(const char *group, int *lo, struct history *h)
{
    ...
    while (1) {
        ...
        default:
            ...
            warn("OVDB: expiregroup: ovdb_getgroupinfo: %s", db_strerror(ret));
            return false;
        }
        ...
        for (i = 0; i < EXPIREGROUP_TXN_SIZE; i++) {
            ...
            if (ret == DB_NOTFOUND || key.size != sizeof dk
                || dk.groupnum != gi.current_gid) {
                done++;
                break;
            }
            ...
            if (delete) {
                if (!compact) {
                    ...
                    default:
                        cursor->c_close(cursor);
                        TXN_ABORT(t_expgroup_loop, tid);
                        warn("OVDB: expiregroup: c_del: %s", db_strerror(ret));
                        return false;
                    }
                }
                if (gi.count > 0)
...
bool
ovdb_ctl(OVCTLTYPE type, void *val)
{
    int *i;
    float *f;
    OVSORTTYPE *sorttype;
