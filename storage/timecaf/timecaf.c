...
**
**  Written by Richard Todd <rmtodd@mailhost.ecn.uoknor.edu> in 1998.
**
**  Various bug fixes, code and documentation improvements since then
**  in 1998-2010, 2014, 2015, 2021, 2022, 2024, 2026.
*/
...
typedef struct {
    ...
    char *mmapbase; /* actual start of mmaped region (on pagesize bndry, not
                       necessarily == artdaya */
...
timecaf_explaintoken(const TOKEN token)
{
    ...
    xasprintf(&text,
              "method=timecaf class=%u time=%lu seqnum=%lu "
              "file=%s/timecaf-%02x/%02x/%02x%02x.CF",
              (unsigned int) token.class,
              ((unsigned long) (ntohl(arrival))) << 8,
              ((unsigned long) ntohs(seqnum1))
                  + (((unsigned long) ntohs(seqnum2)) << 16),
              innconf->patharticles, token.class, (ntohl(arrival) >> 8) & 0xff,
              (ntohl(arrival) >> 16) & 0xff, ntohl(arrival) & 0xff);

    return text;
}

static TOKEN
MakeToken(time_t now, ARTNUM seqnum, STORAGECLASS class, TOKEN *oldtoken)
{
    TOKEN token;
    uint32_t i;
    uint16_t s;
...
static void
BreakToken(TOKEN token, time_t *now, ARTNUM *seqnum)
{
    uint32_t i;
    uint16_t s1 = 0;
    uint16_t s2 = 0;
...
*/
static char *
MakePath(time_t now, const STORAGECLASS class)
{
    char *path;
    size_t length;
    ...
    snprintf(path, length, "%s/timecaf-%02x/%02x/%02x%02x.CF",
             innconf->patharticles, class, (unsigned int) ((now >> 8) & 0xff),
             (unsigned int) ((now >> 16) & 0xff), (unsigned int) (now & 0xff));

    return path;
}

static TOKEN *
PathNumToToken(char *path, ARTNUM artnum)
{
    int n;
    ...
    if (n != 3)
        return (TOKEN *) NULL;
    timestamp =
        ((t1 << 8) & 0xff00) | ((t2 << 8) & 0xff0000) | ((t2 << 0) & 0xff);
    class = tclass;
...
*/
static CAFTOCCACHEENT *
AddTOCCache(time_t timestamp, CAFTOCENT *toc, CAFHEADER head,
            STORAGECLASS tokenclass)
{
    CAFTOCL2CACHE *l2;
    CAFTOCL1CACHE *l1;
    CAFTOCCACHEENT *cent;
    ...
    if (TOCCache[tokenclass] == NULL) {
        ...
        for (i = 0; i < 256; ++i)
            TOCCache[tokenclass]->l2ptr[i] = NULL;
    }
...
TOKEN
timecaf_store(const ARTHANDLE article, const STORAGECLASS class)
{
    char *path;
    char *p;
    time_t now;
...
static ARTHANDLE *
OpenArticle(const char *path, ARTNUM artnum, const RETRTYPE amount)
{
    int fd;
...
ARTHANDLE *
timecaf_retrieve(const TOKEN token, const RETRTYPE amount)
{
    time_t timestamp;
    ARTNUM artnum;
    char *path;
    ...
    /*
    ** Do a possible shortcut on RETR_STAT requests, going through the "TOC
    ** cache" we mentioned above.  We only try to go through the TOC Cache
    ...
    */
    ...
    if ((art = OpenArticle(path, artnum, amount)) != (ARTHANDLE *) NULL) {
        art->arrived = timestamp
                       << 8; /* XXX not quite accurate arrival time,
...
void
timecaf_freearticle(ARTHANDLE *article)
{
    PRIV_TIMECAF *private;
...
DoCancels(void)
{
    if (DeletePath != NULL) {
        if (NumDeleteArtnums != 0) {
            ...
            ** doing CAFRemove...
            */
            if (WritingFile.path != NULL
                && strcmp(WritingFile.path, DeletePath) == 0) {
                CloseOpenFile(&WritingFile);
            }
            /* XXX should really check err. code here, but not much we can
             * really do. */
...
timecaf_cancel(TOKEN token)
{
    ...
    if (NumDeleteArtnums >= MaxDeleteArtnums) {
        ...
        DeleteArtnums =
            xrealloc(DeleteArtnums, MaxDeleteArtnums * sizeof(ARTNUM));
    }
...
FindNextArt(const CAFHEADER *head, CAFTOCENT *toc, ARTNUM *artp)
{
    ...
    while (true) {
        art++;
        if (art > head->High)
            return false; /* ran off the end of the TOC */
...
timecaf_next(ARTHANDLE *article, const RETRTYPE amount)
{
    ...
    while (priv.curtoc == NULL
           || !FindNextArt(&priv.curheader, priv.curtoc, &priv.curartnum)) {
        if (priv.curtoc) {
            free(priv.curtoc);
            priv.curtoc = NULL;
        }
        while (!priv.ter
               || ((priv.terde = FindDir(priv.ter, FIND_CAF)) == NULL)) {
            if (priv.ter) {
                closedir(priv.ter);
                priv.ter = NULL;
            }
            while (!priv.sec
                   || ((priv.secde = FindDir(priv.sec, FIND_DIR)) == NULL)) {
                if (priv.sec) {
                    closedir(priv.sec);
                    priv.sec = NULL;
                }
                if (!priv.top
                    || ((priv.topde = FindDir(priv.top, FIND_TOPDIR))
                        == NULL)) {
                    if (priv.top) {
                        /* end of search */
                        closedir(priv.top);
                    ...
                    if ((priv.topde = FindDir(priv.top, FIND_TOPDIR))
                        == NULL) {
                        SMseterror(SMERR_UNDEFINED, NULL);
                        closedir(priv.top);
                        free(path);
                ...
                snprintf(path, length, "%s/%s", innconf->patharticles,
                         priv.topde->d_name);
                if ((priv.sec = opendir(path)) == NULL)
                    continue;
            }
            snprintf(path, length, "%s/%s/%s", innconf->patharticles,
                     priv.topde->d_name, priv.secde->d_name);
            if ((priv.ter = opendir(path)) == NULL)
                continue;
        }
        snprintf(path, length, "%s/%s/%s/%s", innconf->patharticles,
                 priv.topde->d_name, priv.secde->d_name, priv.terde->d_name);
    ...
    snprintf(path, length, "%s/%s/%s", priv.topde->d_name, priv.secde->d_name,
             priv.terde->d_name);
...
bool
timecaf_ctl(PROBETYPE type, TOKEN *token UNUSED, void *value)
{
    struct artngnum *ann;
...
bool
timecaf_flushcacheddata(FLUSHTYPE type)
{
