...
static void
timeval_normalise(struct timeval *t)
{
    time_t carry;
...
static struct timeval
timeval_difference(struct timeval a, struct timeval b)
{
    struct timeval result;
...
add_client(int sock)
{
    ...
    if (client_count >= client_capacity) {
        ...
        clients =
            xreallocarray(clients, new_client_capacity, sizeof(client_t));
...
#        ifdef USE_DICTIONARY
    if (init) {
        basedict_len = snprintf(dictionary, sizeof dictionary, basedict_format,
                                innconf->pathhost);
        sqlite3_bind_text(sql_main.setmisc, 1, "basedict", -1, SQLITE_STATIC);
        sqlite3_bind_blob(sql_main.setmisc, 2, dictionary, basedict_len,
                          SQLITE_STATIC);
...
static void
pack_length(z_stream *stream, uint32_t length)
{
...
static unsigned int
make_dict(char const *groupname, int groupname_len, uint64_t artnum)
{
    sqlite3_snprintf(sizeof dictionary - basedict_len,
                     dictionary + basedict_len, "%.*s:%llu\r\n", groupname_len,
                     groupname, artnum);
...
open_db(void)
{
        ...
        if (pagesize) {
            ...
            if (default_pagesize != pagesize) {
                ...
                snprintf(sqltext, sizeof sqltext,
                         "pragma page_size=%lu; vacuum;", pagesize);
    ...
    status =
        sqlite_helper_init(&sql_main_helper, (sqlite3_stmt **) &sql_main,
                           connection, SQLITE_PREPARE_PERSISTENT, &errmsg);
    if (status != SQLITE_OK)
...
close_db(void)
{
    if (use_wal) {
        ...
        rc = sqlite3_wal_checkpoint_v2(connection, NULL,
                                       SQLITE_CHECKPOINT_PASSIVE, &wal_frames,
                                       &checkpointed);
...
static bool
finish_request(client_t *client)
{
    return client->request->left == 0;
}

static void
start_response(client_t *client, unsigned int code)
{
    uint8_t code_r;
...
static void
finish_response(client_t *client)
{
...
static void
simple_response(client_t *client, unsigned int code)
{
...
static void
sql_error_response(client_t *client, int status)
{
    buffer_t *respbuf;
    int32_t status_r;
    char const *errmsg;
...
#    define failvar unsigned int code = response_ok
...
static void
do_set_cutofflow(client_t *client)
{
    buffer_t *reqbuf;
    uint8_t cutofflow;
    failvar;
...
do_add_group(client_t *client)
{
    ...
    if (!groupid) {
        ...
        sqlite3_bind_blob(stmt, 1, groupname, groupname_len, SQLITE_STATIC);
        sqlite3_bind_blob(stmt, 2, flag_alias, flag_alias_len, SQLITE_STATIC);
...
static void
do_get_groupinfo(client_t *client)
{
...
static void
do_delete_group(client_t *client)
{
    buffer_t *reqbuf;
    void *groupname;
    uint16_t groupname_len;
...
static void
do_list_groups(client_t *client)
{
    buffer_t *reqbuf, *respbuf;
    uint32_t space;
    int64_t groupid;
...
static void
do_add_article(client_t *client)
{
    buffer_t *reqbuf;
    void *groupname;
    uint16_t groupname_len;
...
#        ifdef USE_DICTIONARY
        status =
            deflateSetDictionary(&deflation, (uint8_t *) dictionary,
...
static void
do_get_artinfo(client_t *client)
{
    buffer_t *reqbuf, *respbuf;
    void *groupname;
    uint16_t groupname_len;
...
static void
do_delete_article(client_t *client)
{
    buffer_t *reqbuf;
    void *groupname;
    uint16_t groupname_len;
...
static void
do_search_group(client_t *client)
{
    buffer_t *reqbuf, *respbuf;
    uint32_t space;
    uint8_t flags, cols;
    ...
    if (!finish_request(client))
        fail(response_bad_request);
    if (space >= 0x100000 || flags & ~search_flags_all
...
static void
do_start_expire_group(client_t *client)
{
    buffer_t *reqbuf;
    void *groupname;
    uint16_t groupname_len;
...
static void
do_expire_group(client_t *client)
{
    buffer_t *reqbuf;
    bool hit_low = false;
    void *groupname;
...
static void
do_finish_expire(client_t *client)
{
    int changes;
    int64_t groupid = 0;
    failvar_stmt_savepoint;
...
/* clang-format off */
static void (*dispatch[count_request_codes])(client_t *) =
{
    do_hello,
    do_set_cutofflow,
    do_add_group,
