    ...
    if (getInteger(topScope, "backlog-highwater", &iv, NO_INHERIT)) {
        if (iv < 0) {
            ...
            logOrPrint(LOG_ERR, fp,
                       ...
                       " than 0. Using %ld",
                       "backlog-highwater", iv, "global scope",
                       (long) TAPE_HIGHWATER);
            iv = TAPE_HIGHWATER;
        }
    } else
    ...
    if (getInteger(topScope, "backlog-rotate-period", &iv, NO_INHERIT)) {
        if (iv < 0) {
            ...
            logOrPrint(LOG_ERR, fp,
                       "ME config: value of %s (%ld) in %s cannot be less"
                       " than 0. Using %ld",
                       "backlog-rotate-period", iv, "global scope",
                       (long) TAPE_ROTATE_PERIOD);
    ...
    if (getInteger(topScope, "backlog-ckpt-period", &iv, NO_INHERIT)) {
        if (iv < 0) {
            ...
            logOrPrint(LOG_ERR, fp,
                       "ME config: value of %s (%ld) in %s cannot be less"
                       " than 0. Using %ld",
                       "backlog-ckpt-period", iv, "global scope",
                       (long) TAPE_CHECKPOINT_PERIOD);
    ...
    if (getInteger(topScope, "backlog-newfile-period", &iv, NO_INHERIT)) {
        if (iv < 0) {
            ...
            logOrPrint(LOG_ERR, fp,
                       "ME config: value of %s (%ld) in %s cannot be less"
                       " than 0. Using %ld",
                       "backlog-newfile-period", iv, "global scope",
                       (long) TAPE_NEWFILE_PERIOD);
...
newTape(const char *peerName, bool dontRotate)
{
    ...
    sprintf(nt->outputFilename, "%s/%s%s", tapeDirectory, peerName,
            OUTPUT_TAIL);
    ...
    if (ckNewFileId == 0 && tapeCkNewFilePeriod > 0)
        ckNewFileId =
            prepareSleep(tapeCkNewFileCbk, tapeCkNewFilePeriod, NULL);
...
void
tapeLogStatus(Tape tape, FILE *fp)
{
    if (tape == NULL)
        fprintf(fp, "(no tape)\n");
...
void
printTapeInfo(Tape tape, FILE *fp, unsigned int indentAmt)
{
    char indent[INDENT_BUFFER_SIZE];
    unsigned int i;
#if defined(INNFEED_DEBUG)
...
getArticle(Tape tape)
{
    ...
    while (tape->inFp != NULL && art == NULL) {
        ...
        if (fgets(line, sizeof(line), tape->inFp) == NULL) {
            ...
            for (p = line; *p && isspace((unsigned char) *p);
                 p++) /* eat whitespace */
                /* nada */;
                ...
                if (q != NULL) {
                    ...
                    *q = '\0';
                    ...
                    if (*q != '\0') {
                        ...
                        filename =
                            NULL; /* line had one field and some blanks */
                } else
...
prepareFiles(Tape tape)
{
    ...
    if (tape->lastRotated == 0) /* first time here */
    {
        ...
        inpExists =
            (tape->inFp != NULL) ? true : false; /* can this ever be true?? */
        outExists =
            (tape->outFp != NULL && tape->outputSize > 0) ? true : false;
    }
    ...
    if (inpExists) {
        ...
        if ((tape->inFp = fopen(tape->inputFilename, "r+")) == NULL)
            ...
            if (fgets(buffer, sizeof(buffer) - 1, tape->inFp) == NULL) {
                ...
                if ((flength = fileLength(fileno(tape->inFp)))
                    < tape->tellpos) {
                    ...
                    if (c == EOF) {
                        ...
                        warn("ME internal checkpoint line boundary missed:"
                             " %s %ld vs. %ld",
                             tape->inputFilename, tape->tellpos, oldPos);
                    }
                }
            }
        }
...
static void
tapeCkNewFileCbk(TimeoutId id, void *d UNUSED)
{
...
static void
tapeCheckpointCallback(TimeoutId id, void *d UNUSED)
{
    ASSERT(id == checkPtId);
