...
For the C<ovdb> overview storage method, you'll need S<Berkeley DB 4.4> or
later (4.7 or later recommended).  For the C<hissqlite> history storage method
or the C<ovsqlite> overview storage method, you'll need S<SQLite 3.24.0> or
later.  If you have zlib available, you can also compress overview before it's
stored into C<ovdb> or C<ovsqlite>, and enable the COMPRESS capability, an
NNTP extension.  For support for news reading over TLS, you'll need OpenSSL or
LibreSSL.  To support SASL authentication to B<nnrpd> or to feed newsgroups to
an IMAP server with SASL authentication, you'll need the Cyrus SASL libraries.
INN can also check passwords against a Kerberos KDC; for this, you will need
Kerberos libraries.
...
so as to pick up possible bug fixes.
...
documentation too.  Please use the GitHub issue tracker to send such
non-security bug reports, or alternatively send them to
...
Security issues must be reported through a
dedicated private form on the L<GitHub Security
Advisories|https://github.com/InterNetNews/inn/security/advisories/> of the
project as they should not be made public until a fix is deployed.
...
If you have a non-security patch or a utility that you'd like to be considered
for inclusion into INN, please make a GitHub pull request, or alternatively
mail it to
...
Patches related to security issues must not be made
public.  A dedicated private form on the L<GitHub Security
Advisories|https://github.com/InterNetNews/inn/security/advisories/> of the
project must be used instead.
...
URL: L<https://github.com/infybofh/cleanfeed-ng/> (maintained by Ivo Gandolfo)

cleanfeed-ng is a maintained, lightweight continuation of the historical
Cleanfeed filter.  It is designed for server-to-server Usenet transit
filtering, with particular attention to predictable behaviour, low overhead,
safe configuration changes, and useful diagnostics.

cleanfeed-ng is an extremely powerful spam filter, probably the most widely
used spam filter on Usenet currently.  It catches excessive multiposting and a
host of other things, and is highly configurable.  Note that it requires the
MD5 Perl module and that INN be built with Perl support (the B<--with-perl>
option to configure).

Since 2026, cleanfeed-ng has continued the historical work of Cleanfeed,
originally developed by Jeremy Nixon, who maintained it until 1998.
Further development was then taken on by Marco d'Itri until 2002, followed
by later updates and maintenance by Steve Crook from 2007 to 2020 (at
L<http://www.mixmin.net/cleanfeed/> but no longer maintained).
...
URL: L<https://github.com/rfc1036/gup>
...
that quickly and reliably streams Usenet articles to a remote site.
