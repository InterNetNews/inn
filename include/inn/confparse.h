...
config_group config_parse_file(const char *filename, /* types */ ...);
...
struct vector *config_params(config_group);
...
/* Used for reporting semantic errors, config_error_param reports the given
   ...
   using warn. */
void config_error_group(config_group, const char *fmt, ...);
void config_error_param(config_group, const char *, const char *fmt, ...);
