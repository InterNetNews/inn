    ...
    while (state != DP_S_DONE) {
        ...
        case DP_S_CONV:
            ...
            case 'o':
                ...
                total += fmtint(buffer, &currlen, maxlen, value, 8, min, max,
                                flags);
                break;
            case 'u':
                flags |= DP_F_UNSIGNED;
            ...
            fallthrough3:
                ...
                if (cflags == DP_C_LDOUBLE)
                    ...
                    fvalue = (LDOUBLE) va_arg(args, double);
                if (max == 0)
                    /* C99 says: if precision [for %g] is zero, it is taken as
                     * one */
                    max = 1;
                total +=
                    fmtfp(buffer, &currlen, maxlen, fvalue, min, max, flags);
            ...
            case 's':
                ...
                total += fmtstr(buffer, &currlen, maxlen, strvalue, flags, min,
                                max);
                break;
            case 'p':
