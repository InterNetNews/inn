...
sub collect($$$$$$) {
    ...
    if ($prog eq "innd") {
        ...
        return 1
          if ($left
              =~ /\S+ status seconds \d+ accepted \d+ refused \d+ rejected \d+ duplicate \d+ accepted size \d+ duplicate size \d+(?: rejected size \d+)?$/o
          );
    ...
    if ($prog eq "innfeed") {
        ...
        return 1
          if $left
          =~ m/\S+:\d+ checkpoint seconds \d+ offered \d+ accepted \d+ refused \d+ rejected \d+/o;
        # final (child)
        return 1
          if $left
          =~ m/\S+:\d+ final seconds \d+ offered \d+ accepted \d+ refused \d+ rejected \d+/o;
        # global (real)
        return 1
          if $left
          =~ m/\S+ global seconds \d+ offered \d+ accepted \d+ refused \d+ rejected \d+ missing \d+/o;
    ...
    if ($prog =~ /^nnrpd(?:-[st]sl)?$/) {
        ...
        return 1
          if $left
          =~ /^python authenticate method succeeded, return code \d+, error string /o;
        return 1 if $left =~ /^python access method succeeded$/o;
        return 1
          if $left
          =~ /^python dynamic method \(\w+ access\) succeeded, refuse string: /o;
        return 1
          if $left =~ /^python: .+ module successfully hooked into nnrpd$/o;
        return 1 if $left =~ /^python: nnrpd .+ class instance created$/o;
        return 1
          if $left
          =~ /^python: n_a authenticate\(\) invoked: hostname \S+, ipaddress \S+, interface \S+, user /o;
        return 1
          if $left
          =~ /^python: n_a access\(\) invoked: hostname \S+, ipaddress \S+, interface \S+, user /o;
        return 1
          if $left
          =~ /^python: n_a dynamic\(\) invoked against type \S+, hostname \S+, ipaddress \S+, interface \S+, user /o;
        ...
        return 1
          if $left
          =~ /^\? reverse lookup for \S+ failed: .* -- using IP address for access$/o;
        # profile timer
        # ME time X nnnn X(X) [...]
        # The exact timers change from various versions of INN, so try to deal
        # with this in a general fashion.
        ...
        return 1
          if $left
          =~ /^\S+ overstats count \d+ hit \d+ miss \d+ time \d+ size \d+ dbz \d+ seek \d+ get \d+ artcheck \d+$/o;
        # starttls
        return 1
          if $left
