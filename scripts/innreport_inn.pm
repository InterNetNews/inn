...
sub collect($$$$$$) {
    ...
    if ($prog eq "inn") {
        ...
        if ($left
            =~ /(\S+) <[^>]+> (?:437|439) Unwanted newsgroup \"(\S+)\"$/o)
        {
            my ($server, $group) = ($1, $2);
            ($group) = split(/,/, $group);
            $server = lc $server unless $CASE_SENSITIVE;
        ...
        if ($left
            =~ /(\S+) <[^>]+> (?:437|439) Unwanted distribution \"(\S+)\"$/o)
        {
            my ($server, $dist) = ($1, $2);
            $server = lc $server unless $CASE_SENSITIVE;
            $inn_badart{$server}++;
        ...
        if (
            $left =~ /(\S+)\ <[^>]+>
                      \ (?:437|439)\ Article\ injected\ or\ posted
                      \ in\ the\ future\ --\ \"[^\"]+\"/ox
        ) {
            my $server = $1;
            $server = lc $server unless $CASE_SENSITIVE;
    ...
    if ($prog eq "innd") {
        ...
        return 1
          if $left
          =~ /^python: First load, so I can do initialization stuff\.$/o;
        return 1 if $left =~ /^python: filter_before_reload executing\.\.\.$/o;
        return 1
          if $left
          =~ /^python: I\'m just reloading, so skip the formalities\.$/o;
        return 1
          if $left =~ /^python: spamfilter successfully hooked into INN$/o;
        ...
        # connected
        #
        # Record <server>:<channel> instead of just <server> as otherwise we
        # may miss some persistent connection newsfeeds has and for which
        # in any given innreport reporting period there may not be any records
        # of connect entries.  We'll accumulate these into totals at the end of
        # processing.
        ...
        return 1
          if (
              $left =~ /\S+\ status\ seconds\ \d+\ accepted\ \d+
                        \ refused\ \d+\ rejected\ \d+\ duplicate\ \d+
                        \ accepted\ size\ \d+\ duplicate\ size\ \d+
                        (?:\ rejected\ size\ \d+)?
                        $/ox
          );
        ...
        if (
            $left =~ /(\S+:\d+)\ (checkpoint|closed)\ seconds\ (\d+)
                      \ accepted\ (\d+)\ refused\ (\d+)\ rejected\ (\d+)
                      \ duplicate\ (\d+)\ accepted\ size\ (\d+)
                      \ duplicate\ size\ (\d+)(?:\ rejected\ size\ (\d+))?
                      $/ox
        ) {
            my (
        ...
        # HISmisses: the entry was not in the cache, but was in the history
        #            file.
        # HISdne:    the entry was not in cache or history.
        if (
            $left =~ m/^ME\ HISstats              # ME HISstats
                       \ (\d+)\s+hitpos           # hitpos
                       \ (\d+)\s+hitneg           # hitneg
                       \ (\d+)\s+missed           # missed
                       \ (\d+)\s+dne              # dne
    ...
    if ($prog eq "innfeed") {
        ...
        # checkpoint (child)
        return 1
          if $left =~ m/\S+:\d+\ checkpoint\ seconds\ \d+
                        \ offered\ \d+\ accepted\ \d+\ refused\ \d+
                        \ rejected\ \d+/ox;
        # final (child)
        return 1
          if $left =~ m/\S+:\d+\ final\ seconds\ \d+\ offered\ \d+
                        \ accepted\ \d+\ refused\ \d+\ rejected\ \d+/ox;
        # global (real)
        return 1
          if $left =~ m/\S+\ global\ seconds\ \d+\ offered\ \d+
                        \ accepted\ \d+\ refused\ \d+\ rejected\ \d+
                        \ missing\ \d+/ox;
        # checkpoint (real) (new format)
        if (
            $left =~ /(\S+)\ checkpoint\ seconds\ (\d+)\ offered\ (\d+)
                      \ accepted\ (\d+)\ refused\ (\d+)\ rejected\ (\d+)
                      \ missing\ (\d+)\ accsize\ (\d+)\ rejsize\ (\d+)
                      \ spooled\ (\d+)/ox
        ) {
            my (
        ...
        if ($left =~ /(\S+) checkpoint seconds (\d+) spooled (\d+)/o) {
            ...
            $server = lc $server unless $CASE_SENSITIVE;
            # Initialize a value for that key (otherwise, it does not appear
            # in the report, and the total line is wrong).
        ...
        return 1
          if $left =~ m/connect : Address family not supported by protocol/o;
    ...
    if ($prog eq "innxbatch") {
        # stats
        if (
            $left =~ /(\S+)\ stats\ offered\ (\d+)\ accepted\ (\d+)
                      \ refused\ (\d+)\ rejected\ (\d+)\ accsize\ (\d+)
                      \ rejsize\ (\d+)
                      $/ox
        ) {
            my (
    ...
    if ($prog eq "innxmit") {
        ...
        if (
            $left =~ /(\S+)\ rejected\ [^\s]+\ \(.*?\)
                      \ (?:437|439)\ Unwanted\ distribution\ \"(\S+)\"
                      $/ox
        ) {
            my ($server, $dist) = ($1, $2);
            $server = lc $server unless $CASE_SENSITIVE;
        ...
        # xx rejected foo.bar/12345 (foo/bar/12345)
        # 437 Unwanted distribution "..."
        if (
            $left =~ /^(\S+)\ rejected\ .*
                      \ (?:437|439)\ Unwanted\ distribution\ \"(\S+)\"
        ...
        if (
            $left =~ /(\S+)\ rejected\ .*
                      \ (?:437|439)\ Newsgroup\ name\ illegal\ --\ "[^\"]*"
                      $/ox
        ) {
            my $server = $1;
            $server = lc $server unless $CASE_SENSITIVE;
        ...
        if (
            $left =~ /(\S+)\ stats\ offered\ (\d+)\ accepted\ (\d+)
                      \ refused\ (\d+)\ rejected\ (\d+)\ missing\ (\d+)
                      \ accsize\ (\d+)\ rejsize\ (\d+)
                      $/ox
        ) {
            my (
    ...
    if ($prog eq "nntplink") {
        ...
        if (
            $left =~ /(\S+)\ .*\ 400\ No\ space\ left\ on\ device
                      \ writing\ article\ file\ --\ throttling
                      $/ox
        ) {
            my $server = $1;
            $server = lc $server unless $CASE_SENSITIVE;
        ...
        # stats
        if (
            $left =~ /(\S+)\ stats\ (\d+)\ offered\ (\d+)\ accepted\ (\d+)
                      \ rejected\ (\d+)\ failed\ (\d+)\ connects
                      $/ox
        ) {
            my ($server, $offered, $accepted, $rejected, $failed, $connects)
              = ($1, $2, $3, $4, $5, $6);
            $server = lc $server unless $CASE_SENSITIVE;
            $nntplink_offered{$server}
              += $offered - $nntplink_ihfail{$server}++;
    ...
    if ($prog =~ /^nnrpd(?:-(?:ssl|tls))?$/) {
        ...
        # python authenticate method succeeded, return code ..., error string
        return 1
          if $left =~ /^python\ authenticate\ method\ succeeded,
                       \ return\ code\ \d+,\ error\ string\ /ox;
        return 1 if $left =~ /^python access method succeeded$/o;
        # python dynamic method succeeded, refuse string:
        return 1
          if $left =~ /^python\ dynamic\ method\ \(\w+\ access\)\ succeeded,
                       \ refuse\ string:\ /ox;
        return 1
          if $left =~ /^python: .+ module successfully hooked into nnrpd$/o;
        return 1 if $left =~ /^python: nnrpd .+ class instance created$/o;
        return 1
          if $left =~ /^python:\ n_a\ authenticate\(\)\ invoked:
                       \ hostname\ \S+,\ ipaddress\ \S+,
                       \ interface\ \S+,\ user\ /ox;
        return 1
          if $left =~ /^python:\ n_a\ access\(\)\ invoked:
                       \ hostname\ \S+,\ ipaddress\ \S+,
                       \ interface\ \S+,\ user\ /ox;
        return 1
          if $left =~ /^python:\ n_a\ dynamic\(\)\ invoked\ against
                       \ type\ \S+,\ hostname\ \S+,
                       \ ipaddress\ \S+,\ interface\ \S+,\ user\ /ox;
        ...
        return 1
          if $left
          =~ /^python: authentication access by IP address succeeded$/o;
        return 1
          if $left =~ /^python: authentication access by IP address failed$/o;
        return 1
          if $left
          =~ /^python: dynamic access module successfully hooked into nnrpd$/o;
        return 1
          if $left
          =~ /^python: dynamic authorization access for read access granted$/o;
        return 1
          if $left
        ...
        return 1
          if $left =~ /^\?\ reverse\ lookup\ for\ \S+\ failed:
        ...
        # other stats
        return 1
          if $left =~ /^\S+\ overstats\ count\ \d+\ hit\ \d+\ miss\ \d+
                       \ time\ \d+\ size\ \d+\ dbz\ \d+\ seek\ \d+
                       \ get\ \d+\ artcheck\ \d+
                       $/ox;
        # starttls
        return 1
    ...
    if ($prog eq "rnews") {
        ...
        if (
            $left =~ /rejected\ (?:437|439)\ Article\ injected
        ...
        if ($left =~ /^offered \S+ (\S+)/o) {
            ...
            # To avoid confusions with real feeds, the first character is
            # forced to be a '3' or a '4'
            # (will work between 9/7/1995 and 13/7/2012).
    ...
    if ($prog eq "controlchan") {
        ...
        # control_XXgroup, foo.bar [moderated] who who token,
        # [pattern], [pattern], encoding, peer, action, 1
        #
    ...
    if ($prog eq "cnfsstat") {
        ...
        if (
            ...
            # If the size changed, invalidate all of our running fill rate
            # stats.
            if (!exists($cnfsstat_size{$buffer})
...
sub adjust($$) {
    ...
    # Fill some hashes
    {
        ...
        foreach my $key (keys(%innd_connect)) {
            $innd_offered{$key}
              ...
              + ($innd_refused{$key} || 0)
              + ($innd_rejected{$key} || 0);
            $innd_offered_size{$key}
              = ($innd_stored_size{$key} || 0)
        ...
        foreach my $key (keys(%innd_connect)) {
            if ($key =~ /^(\S+):\d+$/) {
                ...
                $innd_duplicated_size_sum{$1}
                  += ($innd_duplicated_size{$key} || 0);
                $innd_offered_size_sum{$1} += ($innd_offered_size{$key} || 0);
                $innd_rejected_size_sum{$1}
                  += ($innd_rejected_size{$key} || 0);
            }
        }
    ...
    if (%inn_flow) {
        ...
        $t =~ m/(\d\d):(\d\d)/o;
        $hash_time{$date2}
          = $hash_time{$date} - ($h2 == $h) * 3600 + $1 * 60 + $2;
