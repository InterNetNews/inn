...
schemes on your systems.  For instance, if yescrypt is supported, you can
generate an encrypted password with an argument like C<$y$j9T$YourSalt$>
to Perl crypt function, where C<y> is the prefix for yescrypt, C<j9T> the
parameters passed to crypt_gensalt(3) to generate the hashed password (these
parameters control the yescrypt configuration, and correspond in this example
...
(e.g. a length multiple of 4 for yescrypt) and cryptographic recommendations
...
This function, on those systems that support it, reads from F</etc/shadow>
