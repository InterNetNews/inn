...
pointer is a pointer into an internal buffer in the QIOSTATE object and
is therefore only guaranteed to remain valid until the next call to
B<QIOread> on that object, since the buffer may be overwritten or moved
around at that point.  If EOF is reached, an error occurs, or if the line
is longer than the buffer size (32KiB), NULL is returned instead.  To
distinguish between the error cases, use B<QIOerror> and B<QIOtoolong>.
...
that when B<QIOtoolong> returns true, B<QIOread> has already consumed and
discarded the remainder of the too-long line, up to and including its
terminating newline; the next call to B<QIOread> will therefore return
the following line in the file rather than a partial remainder of the
too-long one.  Users of this library should in general treat long lines
as fatal errors.
...
printing out each line preceded by its offset in the file.
    ...
    for (offset = QIOtell(qp), p = QIOread(qp); p != NULL;
         offset = QIOtell(qp), p = QIOread(qp))
