...
This is the current development version of INN 2.3.  This code is under
...
Please see the INSTALL document for installation instructions.  See the
NEWS file for what's changed from the previous release.
...
innd, the heard of INN, handles NNTP feeding connections directly; UUCP
newsfeeds use rnews (included in INN) to hand articles off to innd.  Other
parts of INN handle feeding articles out to other news servers, most
commonly innfeed (for real-time outgoing feeds) or nntpsend and innxmit
(used to send batches of news created by innd to a remote site via
TCP/IP).  INN can also handle outgoing UUCP feeds.
...
most SysV make programs should work fine.
...
let us know (see L<Contacting the Maintainers> below).
...
recommended), you will need to have PGP installed.  See F<INSTALL> for
more details.
...
L<Contacting the Maintainers> below).
...
=head1 Contacting the Maintainers

We're interested in all bug reports.  Not just on the programs, but on the
documentation too.  Please send I<all> such reports to inn-bugs@isc.org;
even if you post to Usenet, please CC the above address.  If you have
patches for INN, either to fix a bug or to implement a new feature, please
send them to inn-patches@isc.org.

If you have any suggestions for new features in INN, you may want to join
inn-workers@isc.org, the main mailing list for discussion amongst the
developers, and propose them there.  See below for information on how to
subscribe to the INN-related mailing lists.

All other INN mail should go to inn@isc.org.  (Please do I<not> send bug
reports to this address.)
...
The above-mentioned lists, plus inn-committers@isc.org which receives
notice of every change made to INN, are public mailing lists to which you
can subscribe to if you're interested.  To do so, send mail to
majordomo@isc.org with:

    subscribe LIST

in the body, where LIST is one of inn-workers, inn-bugs, inn-patches, or
inn-committers.  (Again, these lists are specifically for development of
INN and are not general help addresses.  If you're interested in asking,
reading, and possibly answering general questions about how to set up and
maintain INN, read news.software.nntp.)
...
goals and accomplishments from the web page at http://www.isc.org/isc/.
