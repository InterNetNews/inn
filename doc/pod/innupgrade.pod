...
=item F<incoming.conf>

=over 2

=item *

Rename I<nolist> and I<noresendid> to respectively I<list> and I<resendid>.
Invert the related boolean values (since S<INN 2.7>).

=item *

Comment out the removed I<comment> and I<email> parameters (since S<INN 2.7>).
...
with the default value C<hisv6>).

=item *

Rename I<nntpactsync> to I<incominglogfrequency> (since S<INN 2.5>).
Rename I<addnntppostingdate> and I<addnntppostinghost> to respectively
I<addinjectiondate> and I<addinjectionpostinghost> (since S<INN 2.6>).

=item *

Remove the I<refusecybercancels> and I<verifycancels> parameters (since S<INN
2.7>).
...
already existing (new in S<INN 2.7>).
...
of B<innfeed> or B<imapfeed> (B<startinnfeed> was removed in S<INN 2.5>).

=item *

Replace the use of B<filechan> with B<buffchan> in unbuffered mode
(B<filechan> was removed in S<INN 2.7>).
...
Rename I<require_ssl> to I<require_encryption> (since S<INN 2.7>).
...
incoming.conf(5), inn.conf(5), inn-secrets.conf(5), newsfeeds(5),
readers.conf(5).
