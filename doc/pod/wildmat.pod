...
B<bool wildmat_simple(const char *>I<text>B<, const char *>I<pattern>B<);>
...
B<wildmat_simple> is identical to B<wildmat> except that neither C<!> nor
C<,> have any special meanting and I<pattern> is always treated as a
...
B<wildmat_poison> works similarly to B<wildmat>, except that C<@> as the
...
If the B<wildmat_simple> interface is used, the matching rules are the
