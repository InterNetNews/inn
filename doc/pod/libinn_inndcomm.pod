...
The B<ICCopen> function creates a Unix-domain datagram socket, binds it to
a newly-created temporary socket path so that it can later receive the
server's reply, and records the path of the server's own control socket
so that commands can be sent to it, if such sockets are supported.
Otherwise, it creates a named pipe for communicating with the server.  It
returns C<-1> on failure or C<0> on success.  This routine must be called
...
on getting the server's reply.  This is done with a select(2) call using
the given timeout, not by catching a signal.  If the timeout is less than
zero, then no reply will be waited for.  The B<SC_SHUTDOWN>, B<SC_XABORT>,
and B<SC_XEXEC> commands do not get a reply either.  If the timeout is a
positive number of seconds, B<ICCcommand> gives up and returns an error
once that many seconds have elapsed without a reply.  The default, which
is also obtained by setting the timeout to C<0>, is to wait indefinitely
for a reply, checking every 2 minutes that the server process is still
running and giving up as soon as it is not.
...
reason of the failure); a C<0> value indicates success.  Replies are
limited to 64KB; if the server's reply cannot be read back correctly (for
instance because it does not fit in that limit), B<ICCcommand> fails and
returns C<-1> instead of returning a truncated buffer.  The possible
values of I<cmd> are defined in the F<inn/inndcomm.h> header file, and
also in the SYNOPSIS of this man page.  The parameters for each command
are described in ctlinnd(8).
...
the one used in the B<ICCreserve> invocation; the value used in the
B<ICCgo> invocation must always match the one used in the B<ICCpause>
invocation.  The return value for all three routines is
