...
The history structure is an opaque handle returned from B<HISopen>.
...
methods are defined at build time; the history methods currently
available are C<hisv6> and, when INN is built with SQLite support,
C<hissqlite>.  On success, a newly initialised history handle is
...
mmap()ed into the process's address space.  B<HIS_INCORE> is typically
...
B<HISsetcache> associates a cache used for speeding up B<HIScheck> with
...
I<reason> string is used as the argument to the C<ctlinnd pause>
...
If I<exists>() returns B<false>, it indicates that the stored entity
associated with the token is no longer available (or no longer required),
...
as an argument to the C<ctlinnd (reserve|pause|go)> commands.  If
...
Set a hint to the underlying history manager as to how many
...
For the C<hisv6> history method and tagged hash managers, set the interval, in
...
=item C<HISCTLG_INPLACEEXPIRE> (bool *)

Indicate whether the history backend expires in place (mutating the live
database via a writer rather than rebuilding a new file and swapping it).
Backends that do not implement this selector leave the value untouched,
so the caller must initialise it to false.  I<val> should be a pointer to a
value of type B<bool>; backends that do implement it will set it to reflect
their behaviour.  expire(8) uses this to decide whether to open the history
read/write (in place) or read-only (rebuild-and-swap, the hisv6 model).
...
B<HISlogto> enables logging of history performance statistics (as used by
B<innd>'s C<stathist> configuration and ctlinnd(8) directive) to the file
named I<s>, opening it for appending and resetting the internal timing
counters; any previously open log is closed first.  B<HISlogclose> closes
that log, if one is open, and disables further logging.  These two
functions are unrelated to a specific history handle and control a
process-wide logging state used by the history API's internal timers.
