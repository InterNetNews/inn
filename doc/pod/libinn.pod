...
=head1 SYNOPSIS
    ...
    extern FILE *CA_listopen(char *pathname, FILE *FromServer,
                             FILE *ToServer, const char *request);
    ...
    extern bool inn_lock_file(int fd, enum inn_locktype type,
                              bool block);
    extern bool inn_lock_range(int fd, enum inn_locktype type,
                               bool block, off_t offset, off_t size);
    ...
    /* UTF-8 validation. */
    extern bool is_valid_utf8(const char *text);
...
in DNS).  If no valid domain name can be found for the right-hand side,
NULL is returned.
...
B<verify_cancel_key> expects pointers to Cancel-Key and Cancel-Lock header
...
B<CA_listopen> is the lower-level routine used by B<CAlistopen> to implement
the LIST command exchange.  It sends the same LIST command (using I<request>
the same way as B<CAlistopen>), but writes the results to I<pathname>, which
it creates, rather than to a temporary file of its own choosing, and returns
a C<FILE> opened for reading on that path, or NULL on error.  Calling it
directly, rather than through B<CAlistopen>, is only useful for a caller
that wants to control the destination path itself; it is otherwise
considered an internal routine.
...
B<inn_lock_range> does the same, but locks (or unlocks) only the I<size>
bytes starting at I<offset> bytes into the file, instead of the whole
file.  As with fcntl(2) record locking, on which it is implemented, a
I<size> of C<0> means to lock through the end of the file, growing with
it if it is later extended; B<inn_lock_file> is implemented as a call to
B<inn_lock_range> with I<offset> and I<size> both set to C<0>, thus
...
F<include/config.h> is not defined, it falls back to B<NNTPconnect> with
C<127.0.0.1> as the host and the I<port> parameter set in F<inn.conf>, for
compatibility with systems that lack Unix-domain stream sockets.
...
B<HashMessageID> returns a hashed Message-ID using MD5.  Before hashing, the
part of the Message-ID after the arobase (that is to say, its domain part)
is folded to lowercase, so that two Message-IDs differing only by the case
of their domain part hash to the same value.  As a special case, if the
local part is C<postmaster> (case-insensitively), the whole Message-ID is
folded to lowercase.

=head2 UTF-8 VALIDATION

B<is_valid_utf8> checks whether the nul-terminated string I<text> is valid
UTF-8.  Each byte sequence must either be a single-byte, printable ASCII
character (or one of carriage return, newline or a horizontal tab), or a
well-formed multi-byte UTF-8 encoding of two to six bytes, with each
continuation byte matching the C<10xxxxxx> pattern.  Any other byte
sequence, including other ASCII control characters, causes B<is_valid_utf8>
to return false; otherwise, it returns true.
...
the total user and system time used by the current process.  If
C<HAVE_GETRUSAGE> in F<include/config.h> is defined, it gets the values by
doing a getrusage(2) system call for the current process only; otherwise it
calls times(2), which also adds in the time used by any children of the
current process that have already been waited for.  It returns C<-1> on
...
on 64-bit Solaris, low-numbered file descriptors can then be kept and reused
for stdio.  Without this mechanism, some essential files like the history
database file may not have any file descriptor left when being reopened after
a closure for some operations, and stdio would fail.
...
reusing a reserved file descriptor.  It returns a pointer to a FILE struct,
or NULL on failure.
...
C<0> on success, and C<-1> on failure (when setrlimit(2) is not available or
