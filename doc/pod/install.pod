...
your hostnames are not fully-qualified (i.e., your systems are named
"host" instead of "host.example.com") then you will need to use this
option to set your domain name manually.  If in doubt, leave this option
commented out (or remove it completely).  See I<domain> in readers.conf(5)
if you want to affect the righthand side of autogenerated Message-IDs.
...
it requires your OS support mmap(2) on a block device.  Solaris supports
...
many cycbuffs into each metacycbuff.  Some versions of Linux are known to
...
OSes, if you want cycbuffs over 2 GB, you'll have to use regular files.
If in doubt, keep your cycbuffs smaller than 2 GB.  Also, when laying out
...
all (since INN is capable of using the devices in F</dev>), but you
