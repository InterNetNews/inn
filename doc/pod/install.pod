...
are two overview mechanisms to choose from, tradindexed and buffindexed.
tradindexed very fast for readers, but it has to update two files for
every incoming article and can be quite slow to write.  buffindexed can
keep up with a large feed more easily, since it uses large buffers to
store all overview information in, but it's somewhat slower for readers
(although not as slow as the unified overview in INN 2.2).
