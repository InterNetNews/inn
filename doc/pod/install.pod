...
complete files.  If you plan to only carry a small set of groups, create
F<active> containing only the following lines:
...
to initialize F<active.times>.

FIXME: Currently, INN returns errors to clients if F<active.times> doesn't
