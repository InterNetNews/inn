...
versions you'll need:
    ...
    --with-blacklist    FreeBSD 11 or higher base system
...
Enables nnrpd integration with FreeBSD's blacklistd for blocking of
brute force attacks. blacklistd and its client library libblacklist is
available in the FreeBSD base system since FreeBSD 11 and should be
automatically detected if PATH is not given.

For this feature to have any effect, blacklistd needs to be configured
and running. See the nnrpd documentation for a short configuration
snippet and/or read the blacklistd documentation.
...
header field of the message, but this is obviously perilous and control messages
