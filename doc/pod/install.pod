...
even if you have regular login accounts on your news server, nearly all
news readers these days handle posting through a network connection and
don't need a local setgid inews to post.  Installing inews setgid was the
default prior to INN 2.3.

=item B<--with-openssl=PATH>

Enables support for SSL for news reading, which means it is possible to
have SSL or TLS encrypted NNTP connections between your server and
newsreaders.  This option requires OpenSSL.  If a path is given, it sets
the installed directory of OpenSSL.  You must then make a certificate and
private key.
...
files don't support block access to large raw partitions (Solaris is in
this category); on those OSes, if you want cycbuffs over 2GB, you'll have
to use regular files.  If in doubt, keep your cycbuffs smaller than 2GB.
...
artcutoff parameter in F<inn.conf>, defaulting to 14); the number on the
