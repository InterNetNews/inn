...
them as guides for doing a clean installation and configuration of 2.8.
...
the binaries, scripts, and manual pages by running:
...
If you are building INN from a Git clone of the development tree rather than
a snapshot or standard release, you first need to (re)generate the autoconf
files so as to pick up possible changes since the last pull of the Git tree.
This can be done by running the following commands:

    make maintclean
    ./autogen

before building INN with C<./configure> (followed by the wanted configure
options) and C<make>.  You may then also want to run the test suite with
C<make tests>.
...
=head1 Packages and Docker Images

Instead of building INN from sources, you may want to check that a readily
available package does not already exist for your operating system.
...
INN has been confirmed to build on recent versions of the following operating
systems:

    AIX (including 7.2 and 7.3)
    FreeBSD (including 12.1, 13.0, 14.0, 15.0 and 16.0)
    Linux (including 3.16, 4.19, 5.10, 5.15, 6.1, 6.12 and 7.0 kernels,
           glibc 2.19, 2.28, 2.31, 2.36, 2.41 and 2.42,
           and musl 1.1.24, 1.2.2, 1.2.3, 1.2.4, 1.2.5 and 1.2.6)
    macOS (including 11, 12, 13, 14, 15 and 26)
    NetBSD (including 9.2 and 10.1)
    OpenBSD (including 6.8, 7.8 and 7.9)
    Oracle Solaris (including 10 1/13, 11.3 and 11.4)
...
If you want to use the Python embedded hooks, you'll need to have a
...
versions you'll need:
    ...
    --with-blocklist    blocklistd from FreeBSD 11, NetBSD 7 or higher versions
...
L<https://downloads.isc.org/isc/inn/>.  New major releases will be announced
on <inn-announce@lists.isc.org> (see F<README>) when they're made.

If you want a more cutting-edge version, you can obtain current snapshots
from L<https://downloads.isc.org/isc/inn/snapshots/>.  These are snapshots
...
The history database and the F<active> file will be stored in this directory,
and writes to those files are an appreciable percentage of INN's disk
activity.  The history database can also be quite large (requiring up to
S<2 GB> or more), and is rewritten to a new one during nightly B<expire> for
the C<hisv6> history storage method, so this is a common portion of INN to
move to a different file system.  To move only the history database, without
the other database files, see the I<pathhistory> parameter in inn.conf(5).
You can also set a dedicated path for the history database with the
B<--with-history-dir> option given to C<configure>.
...
Enables large file support, arranging for 64-bit file offsets.  This flag
is only useful for 32-bit platforms and some very rare 64-bit platforms that
define a 32-bit I<off_t> type.

This is not enabled by default (unless of course I<off_t> is already 64-bit
long), even on platforms that support it, because it changes the format
of INN's on-disk databases (making it difficult to upgrade an earlier INN
installation) and can significantly increase the size of some of the C<hisv6>
history database files.  Large file support is not necessary unless you use
the C<hisv6> history storage method and your C<hisv6> history database is so
large that it exceeds S<2 GB> or you want to use CNFS buffers larger than S<2
GB> on 32-bit platforms.

The C<hisv6> history, tradindexed and buffindexed overview, and timecaf
databases written by an INN built with this option are incompatible with those
written by an INN without this option.  (CNFS buffers are compatible.)

=item B<--enable-tagged-hash>

Use tagged hash table for the C<hisv6> history database.  The tagged
hash format uses much less memory but is somewhat slower.  This option is
recommended if you have less than S<256 MB> of RAM on your news server.
If you install INN without tagged hash (the default) and B<expire> takes
an excessive amount of time, you should make sure the RAM in your system
...
hash enabled or switch to the C<hissqlite> history storage method.
...
been in the NetBSD base system since S<NetBSD 7>.  Formerly known as blacklist
back then, blocklist is the new name in S<FreeBSD 15> and S<NetBSD 10>.
...
Disadvantages: Needs a faster file system and I/O system than the cnfs
...
your article spool.  As one of the newer and least widely used storage types,
timecaf has not been as thoroughly tested as the other methods.  It requires
running a nightly B<expire> program to delete old articles by either
compacting CAF files if they still contain available articles, or removing
them.
...
with the best recovery tools (B<tdx-util>).
...
used history file entries.  Setting this to C<0> disables history caching.
...
In order to use any cycbuff larger than S<2 GB> on 32-bit platforms (and some
very rare 64-bit platforms that define a 32-bit I<off_t> type), you need to
build INN with the B<--enable-largefiles> option.  See L<Installing INN> for
more information and some caveats.
...
The seventh and eighth parameters are used to better filter spam articles and
store them in dedicated storage classes.  See the storage.conf(5) manual page
...
already expired in the history database so that the server doesn't accept them
...
spool, you want to keep them around in your history database for a little
...
unless the I<pathhistory> parameter puts it elsewhere, the history
database (a record of every article the server currently has or has seen
in the past few days, used to decide whether to accept or refuse new
incoming messages).
...
from that server into I<pathdb>.  Otherwise, you'll need to figure out what
...
go to L<https://downloads.isc.org/pub/usenet/CONFIG/> and download F<active>
and F<newsgroups> from there; that will start you off with reasonably complete
files.  If you plan to only carry a small set of groups, the default minimal
F<active> file installed by INN is a good place to start; you can create
additional groups after the server is running by using C<ctlinnd newgroup>.
(Another option is to use actsync(8) to synchronize your newsgroup list to
that of another server.)
...
Next, you need to create an empty history database.  See I<hismethod> in
F<inn.conf>, and then set appropriately the history storage method you wish
to use.

To create an empty C<hisv6> history database, type:

    cd <pathhistory in inn.conf>
    touch history
    makedbz -i -o

B<makedbz> will then create a database optimized for handling about
6,666,666 articles (or 500,000 if the slower tagged hash format is used).  If
you expect to inject more articles than that, use the B<-s> flag to specify the
number of entries to size the initial C<hisv6> F<history> file for.  To
pre-size it for 100,000,000 articles, type:
...
F<history.sqlite> database in I<pathhistory>.  Do not create the file by hand
...
L<https://downloads.isc.org/pub/usenet/CONFIG/control.ctl>.
...
F<control.ctl>.  L<https://downloads.isc.org/pub/pgpcontrol/PGPKEYS> tries to
