...
not required but is highly recommended) then you will need to install some
version of PGP.  The recommended version is GnuPG, since it's actively
developed, supports OpenPGP, is freely available and free to use for any
purpose (including in the US), and (as of version 1.0.4 at least) supports
RSA signatures used by most current control message senders.  You can get
GnuPG from <http://www.gnupg.org/>.
...
F<README.python_hook> and F<README.python_auth_hook> for all the details.
Note that there is an incompatibility between INN and Python 2.0 when
Python is compiled with cycle garbage collection; this problem was
reported fixed in Python 2.1a1.
...
Here's a command that will print out the dd commands that you should run:
