...
INN should be sufficient.  (If GNU make isn't installed as "make", fix the
path in F<site/do-subst.sh> to point to GNU make.)

FIXME: Ideally, we need to either decide to stop supporting subst entirely
and get rid of this, or add magic to configure to take care of this for
us.
