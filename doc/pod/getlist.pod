...
C<active.times>, C<distributions>, or C<newsgroups>.  C<active> contains
a list of all newsgroups carried by the server with the high and low article
numbers in the group and the group status flag.  C<active.times> is a list of
...
active.times(5), distributions(5) and newsgroups(5).
