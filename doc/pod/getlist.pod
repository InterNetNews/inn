...
The listing files other than the F<active> file are common extensions to
the NNTP protocol and may not be available on all servers.  For instance,
C<counts>, C<distributions>, C<headers>, C<moderators>, C<motd>,
C<overview.fmt> and C<subscriptions>, amongst others, may be available.
Moreover, a I<uwildmat> pattern I<pattern> may also be usable for some of
these listing files.
...
The following command lists all groups that allow local postings, are
