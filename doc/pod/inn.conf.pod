...
How long (in seconds) pre-committed message-ids are kept before unused.
Pre-commit cache is used to respond to peers with ``436'' which means
innd does not want the article currently and requests to try later.
The default value is <5>.

=item I<wipexpire>

How long (in seconds) pre-committed message-ids are kept before expired.
This is used to respond with ``436'' on pending channel before finishing
transfer.  The default value is <10>.
...
I<ovmethod> needs to be set, if I<enableoverview> is set to true.
This is a boolean value and the default is true.

=item I<groupbaseexpiry>

Whether to enable newsgroup base expiry.  If set to false, expiry is
done based on class of storing method.  If set to true and overview
data is created, expiry is done based on newsgroup name.
This is a boolean value and the default is true.
...
writing overview files (see I<enableoverview>), C<tradindexed> is
set for I<ovmethod> and this is set to a value
other than C<0>, INN will keep around and open that many recently
...
C<limit> command to see how many open file descriptors your operating
...
default value is C<15> (which is probably way too low if you have a large
number of file descriptors available).

=item I<ovgrouppat>

For which newsgroups should overview be stored.  If nothing is specified,
overview data for all newsgroups will be stored.  If something is specified,
it is treated as a comma separated list of wildmat and any of matched overview
data will be stored.  Note that if I<groupbaseexpiry> is true, I<ovgrouppat>
is specifed and article storage method does not have self expire functionality,
storing overview data will fail.
This is a boolean value and the default is false.

=item I<ovmethod>

What overview storage method to use.  Currently supported values
are C<tradindexed>, C<buffindexed>, and C<ovdb>.
There is no default value; this parameter must be set if
I<enableoverview> is true (the default).
...
defined in F<buffinedexed.conf>.  C<buffindexed> never consumes disk
space other than those files for overview data and index.

=item C<ovdb>

stores data into a BerkeleyDB database.  See the ovdb(5) man page.
...
Whether to use the Python hook in nnrpd(8) to authenticate readers.  If this
is enabled, normal readers.conf(5) authentication will not be used, and
instead the python hook will be called to authenticate connections.  This is
a boolean value and the default is false.
