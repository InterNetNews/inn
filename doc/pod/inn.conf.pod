...
writing overview files (see I<enableoverview>) and this is set to a value
...
is C<tradindexed>, which stores two files per newsgroup, one containing
the overview data and one containing the index.  There is no default
value; this parameter must be set if I<enableoverview> is true (the
default).
