...
field of C<cnfs> entries in F<storage.conf>.  Unlike a cycbuff name, its
length is not checked when F<cycbuff.conf> is parsed, but it is stored
internally in a fixed-size buffer, so it is recommended to keep it no
longer than eight characters, the same limit as a cycbuff name.
<buffer> is the name of a cycbuff (the <name> part of a cycbuff line),
and any number of cycbuffs may be specified, separated by commas.
...
accordingly, and then restarting B<innd>.  (Only the cycbuff is
