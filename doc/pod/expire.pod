...
B<expire> scans the history database and uses the information recorded in it
to purge itself of old news articles.  Its behaviour depends on the setting of
I<groupbaseexpiry> in F<inn.conf>.
...
Regardless of the setting of I<groupbaseexpiry>, B<expireover> should be run
...
If the B<-d> flag is used with the C<hisv6> history method, then the new
F<history> file and database is created in the specified directory I<dir>.
This is useful when the filesystem does not have sufficient space to hold both
the old and new history files.  When this flag is used, B<expire> leaves the
server paused and creates a zero-length file named after the new history file,
...
This option is not available with the C<hissqlite> history method.
...
To specify an alternate input history database, use the B<-h> flag.  With a
C<hisv6> history method, B<expire> uses the old I<dbz> database to determine
the size of the new one.  (If the B<-d> flag is not used, the output filename
will be the same as the input filename with an extension of C<.n>.)

The default without the B<-h> flag is I<pathhistory>/F<history> for C<hisv6>
and I<pathhistory>/F<history.sqlite> for C<hissqlite>.

=item B<-i>

To ignore the old database for the computation of the estimated size of the
new C<hisv6> history database, use the B<-i> flag.

It has no effect with the C<hissqlite> history method.
...
This flag is only useful for the C<hisv6> history method as C<hissqlite> does
not need pausing B<innd>.
...
when it needs exclusive access to the history database, using the string
...
and the new history database is ready, it sends a C<go> command.  See
also the B<-n> flag.
...
B<expire> determines the optimal size of the new C<hisv6> F<history> file from
the size of the old one.  In case you want to force a specific size, use this
flag to optimize the new history database for approximately I<size> key-value
pairs (i.e. lines in F<history>).  Accurately specifying the size will create
a more efficient database.  (The size should be the estimated eventual number
of articles, typically the size of the old F<history> file, in lines.)

It has no effect with the C<hissqlite> history method.

=item B<-t>

If the B<-t> flag is used, then B<expire> will generate a list of the tokens
that should be removed on its standard output.

With the C<hisv6> history method, the new F<history> file will be left in
F<history.n>, F<history.n.dir>, F<history.n.index> and F<history.n.hash>.
This flag is useful for debugging when used with the B<-n> flag.  Note that
if the B<-f> flag is used, then the name specified with that flag will be used
instead of F<history>.

With the C<hissqlite> history method, it acts as a dry run, reporting what
would be removed without changing the history database or the spool.
...
actions done (not valid if a new history database is not written), level two
...
If the B<-x> flag is used, then B<expire> will not create any new C<hisv6>
history files.  This is most useful when combined with the B<-n> and B<-t>
flags to see how different expiration policies would change the amount of disk
space used.
...
When I<expiretombstone> and I<groupbaseexpiry> are enabled in F<inn.conf>,
B<expire> consumes the per-cycle deletion log produced by B<expireover>,
B<innd>, and B<sm -r> (F<expireover.tombstone> and F<cancels.tombstone> log
files in I<pathdb>) so it can drop history entries for those articles without
doing a per-article C<SMretrieve(RETR_STAT)> syscall.  An empty tombstone is
treated as "no cancels this cycle" and the slow scan is skipped entirely.
See inn.conf(5) under I<expiretombstone> for the file lifecycle, locking
model, and recovery story.
