...
When I<expiretombstone> is enabled in F<inn.conf>, B<expire> consumes the
per-cycle deletion log produced by B<expireover>, B<innd>, and B<sm -r>
(F<expireover.tombstone> and F<cancels.tombstone> log files in I<pathdb>) so
it can drop history entries for those articles without doing a per-article
C<SMretrieve(RETR_STAT)> syscall.  An empty tombstone is treated as "no
cancels this cycle" and the slow scan is skipped entirely.  See inn.conf(5)
under I<expiretombstone> for the file lifecycle, locking model, and recovery
story.
