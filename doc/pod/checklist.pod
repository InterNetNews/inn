...
as any users, because C<make install> will set the permissions
...
For bash and my system, this is:
    ...
    PATH=~news/bin:$PATH
    export PATH
    MANPATH=~news/man:/usr/share/man:/usr
    export MANPATH
...
aren't working out F</usr/local/news>, or want to put some files on a
