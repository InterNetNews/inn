...
accepted articles, and manages the active(5) and history(5)/hissqlite(5)
...
Normally, B<innd> will flush any changes to C<hisv6> F<history> and F<active>
files after 300 seconds of inactivity.  This option changes that timeout to
I<seconds>.

This setting is not used for the C<hissqlite> history method whose writes are
in autocommit mode.
...
is as often as not specified (adding this pattern does not prevent control
...
active(5), ctlinnd(8), hissqlite(5), history(5), incoming.conf(5),
inn.conf(5), innbind(8), innfeed(8), innstat(8), libinn_dbz(3),
libinn_inndcomm(3), newsfeeds(5), nnrpd(8), rnews(1), syslog(3).
