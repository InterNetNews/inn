...
current status of B<innd> can be obtained by running C<ctlinnd status>, or
...
shut down connections.  The mode is normally changed via ctlinnd(8),
either by various automated processes such as nightly article expiration
or by the news administrator, but B<innd> will also throttle itself if it
encounters ENOSPC errors in writing data or an excessive number of I/O
...
incoming.conf(5), which specifies what remote sites B<innd> will accept
connections from, newsfeeds(5), which specifies what is to be done with
incoming articles besides storing them, and inn.conf(5), which sets a wide
...
B<innd> should normally never be run directly.  Among other problems, it
...
must be opened by root.  Instead, B<innd> should be started via
inndstart(8), a small setuid root program that opens the appropriate port,
cleans up the environment, changes to the news user, and then runs
B<innd>, passing along any command-line arguments it is given to B<innd>.
...
value of I<artcutoff> in inn.conf(5).  This option, if given, overrides
...
B<innd> normally puts itself into the background, repoints its standard
...
debugging.  Using B<-f> prevents the backgrounding and disassociation
...
the server per the time interval given by B<-X>.  The default is 2.

The B<-T> flag limits the total number of incoming connections per the
time interval given by B<-X>.  The maximum value is 128, and the default
is 60.
...
given, overrides the value of that setting.  If I<count> is 0, this check
is suppressed.

=item B<-I> I<address>

Normally if B<innd> binds to a port itself, it lets the operating system
...
The IP address must be in dotted quad (nnn.nnn.nnn.nnn) format.

This option is rarely useful since B<innd> should not be binding to a
port.  Instead, use inndstart(8) and the equivalent B<-I> option to it.
...
I<size> is 0, this check is suppressed.

=item B<-m> I<mode>

Normally B<innd> starts in the running mode.  If this option is given, it
specifies what mode B<innd> should start in.  I<mode> should begin with
one of C<g>, C<p>, or C<t>, and the starting mode will be set to running,
paused, or throttled respectively based on that initial letter.  (C<g> is
short for C<go>.)
...
feeds is fixed, being the number of file feeds configured in newsfeeds(5),
...
specifies the port that B<innd> should bind to.  Normally one should never
use this option and instead use B<inndstart> and its equivalent B<-P>
option.  Since B<innd> should never be run as root, I<port> has to be a
...
messages."  Except for cancel messages, these messages are implemented by
an external program, either scripts in the I<pathcontrol> directory (as
set in F<inn.conf>) or by controlchan(8) if I<usecontrolchan> is set to
true in F<inn.conf>.  Use of controlchan(8) is highly recommended, since
otherwise a flood of control messages can cause a denial of service
attack.  Support for control message processing via means other than
controlchan(8) should be considered obsolete and may be removed in future
versions of INN.

If I<usecontrolchan> is false, then when a control message arrives the
first word of the control header is convered to lowercase and used as the
name of the program to execute.  If the named program does not exist, then
I<pathcontrol>/default is executed instead.  All control programs are
invoked with four parameters:

=over 4

=item 1.

The address of the person who posted the message.  This is taken from the
Sender: header if present and the From: header if it is not.

=item 2.

The address to which to send replies, taken from the Reply-To: header.

=item 3.

The storage API token for the article.

=item 4.

The host that sent the article, as specified in the Path: header.

=back

This method of control message handling should be considered obsolete and
not relied on.  Support for it may be removed in future versions of INN in
favor of requiring all control message processing be done via
controlchan(8), which is much safer and more reliable.
...
given remote site (normally a bad idea), add C<control.cancel> to their
...
is taken to be a site name, as configured in newsfeeds(5), and the article
...
innxbatch(8) and the sendxbatches program for more details on this
extension.
...
ID.  An attempt will be made to cancel that message ID, and the server
will reply 289 for success or 484 for failure.  (Failure can occur in
circumstances such as the server being paused.)

=head1 LOGGING

B<innd> reports all incoming articles in its log file.  This is a text
...
depending on the settings of I<nntplinklog> and I<logsize> in F<inn.conf>.
...
where C<%s>, above, is replaced by more specific information.
