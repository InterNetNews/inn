...
B<simpleftp>, mostly used by B<actsyncd>, now correctly parses the port
number following a hostname (separated by a colon).  FTP sessions also
default to passive mode as this mode works better behind firewalls and NAT
configurations.
...
and useful C<Perl::Critic> policies.
