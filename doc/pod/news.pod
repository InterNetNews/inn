...
C<:bytes> and C<:lines> metadata.

=back

=head1 Changes in 2.7.1

=over 2

=item *

Added a new I<groupexactcount> parameter in F<readers.conf> to force
B<nnrpd> to recount and report the exact number of still existing articles
in newsgroups considered to have an estimate number of articles below
I<groupexactcount> (set to C<5> by default).  The primary goal of this recount
is to advertise newsgroups as empty when they are (articles that have been
cancelled or self-expired may otherwise still be in counted in the estimate).
...
channel> command that was returning partial information for the channel.
...
This new separate configuration file to parameter the display will now permit
an automatic update (if of course you use the default display configuration
file).
