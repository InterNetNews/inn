=head1 Upgrading from 2.7 to 2.8
...
network filesystems such as NFS when B<innd> and B<nnrpd> are not exclusively
run on one mount consumer.  In case you use the ovsqlite overview method with
an overview database shared on a network filesystem between two different
hosts, you need manually setting to false the I<walmode> parameter in
F<ovsqlite.conf>.

=item *

If you wish to take benefit from the new I<pathhistory> parameter in
F<inn.conf> that permits configuring a dedicated directory for the history
database, previously necessarily in I<pathdb> amongst other various files,
you are encouraged to set it, and manually move the history files to the
corresponding directory while INN is not running.  Separating history is
especially useful with the new hissqlite history storage method on ZFS, where
the dataset I<recordsize> should match the SQLite page size; see hissqlite(5)
for more details.

=item *

You may also take benefit from the new hissqlite history storage method based
on SQLite.  See hissqlite(5) and hissqlite-convert(8) for more information and
how to migrate from an existing hisv6 history database.

=back

If you are upgrading from a version prior to S<INN 2.7>, see also
L<Upgrading from 2.6 to 2.7>.
...
C<hissqlite>, selectable instead of C<hisv6> via the I<hismethod>
parameter in F<inn.conf> when INN is built with SQLite support.  The
B<hissqlite-convert> program permits migrating an existing C<hisv6> history
database.  See hissqlite(5) for more details.  New I<hissqlitecachesize>,
I<hissqlitemmapsize>, I<hissqlitepagesize> and I<hissqlitereadercachesize>
parameters have also been added to F<inn.conf> to parameterize this new
history storage method.  Many thanks to Kevin Bowling for this great
contribution!

=item *

The new hissqlite history storage method is accompanied with a history
manipulation utility, B<hissqlite-util>, also contributed by Kevin Bowling,
that performs some basic consistency checks and dump operations on a history
...
A new I<pathhistory> parameter has been added to F<inn.conf> for the directory
that holds the history database files (the history(5) text file and its
I<dbz> indices with the C<hisv6> history method, or F<history.sqlite> and
its WAL files with C<hissqlite>).  The default is the value of I<pathdb>, so
existing installations are unchanged if the parameter is left unset.  For new
installations, it can be set at configure time with the B<--with-history-dir>
passed to C<configure>.  Cancel and expireover tombstone logs stay under
I<pathdb> (they are expiry state, not history files).  Thanks to Kevin Bowling
for this feature.

=item *

The I<walmode> parameter in F<ovsqlite.conf> is now enabled by default for the
ovsqlite overview method.
...
Support for old insecure OpenSSL and LibreSSL versions have been dropped.
...
includes buffer growth and concatenation in B<innfeed>, IMAP and LMTP response
parsing, reserved file descriptor management, and hexadecimal and uuencoded
data handling.  Reader tracking and virtualhost permission state, batch
error cleanup, and file-list growth in expiration tools were also corrected.
Many thanks to Kevin Bowling for the patches.
...
database and journal file sizes, and active, deleted and total overview record
counts.  Thanks to Kevin Bowling for this enhancement.
...
B<innreport> now recognizes the logs of cleanfeed-ng and Postfilter-NG, the
maintained continuation of the historical Cleanfeed and Postfilter filters,
respectively for B<innd> and B<nnrpd>.  Dedicated sections are also generated
in daily Usenet reports to summarize audit and rejection events.  Thanks to
Ivo Gandolfo for the feature and naturally also for going on maintaining these
great Perl filters.

=item *

Fixed an unexpected decrease of the low and high water marks of empty
newsgroups when expiring overview data stored with the tradindexed overview
method.  Thanks to Benoit Izac for the bug report.

=item *

Fixed the computation of accepted, duplicate and rejected sizes by B<innd> on
long-running connections because of a float-precision issue.  Thanks to Jesse
Rehmer for the bug report.

=item *

Fixed the output of C<ctlinnd name> for exploder channels.

=item *

When I<laxmid> is set in the I<syntaxchecks> parameter in F<inn.conf>,
Message-IDs without any C<@> character are now accepted by B<innd>.  It was
not always the case in the previous version, though mentioned in the release
notes.  It is now actually fixed, and I<laxmid> even accepts any allowed
printable characters, no matter the order they appear in Message-IDs.

=item *

Fixed about twenty correctness and resource-lifetime defects in several
programs thanks to Kevin Bowling.  They notably address stale pointers after
buffer growth in B<innxmit>, descriptors and heap ownership in B<innxbatch>
and B<innbind>, stale mode reasons in B<expire>, deterministic partial
encoding in the C<c7unbatch> batch processor for B<rnews>, trusted-input
bounds checks and one past-channel descriptor in B<innd>, various leakages
in the external authentication helper of B<nnrpd>, and IMAP/LMTP connection
lifecycle problems in B<imapfeed>.

=item *

Hardened several older code paths against memory-safety failures.  This
includes bounded parsing of overview records during expiration, validation
of corrupt OVDB records and search handles, robustness of the configuration
parser, properly resetting timecaf resources after failed operations, detecting
malformed stored Xref records with tradspool and corrupt history records.
The overview and storage backends also validate more structures before using
them.  Many thanks to Kevin Bowling for the patches.

=back

=head1 Changes in 2.7.4 (2026-06-22)

=over 2

=item *

The speed of B<expireover> has been greatly improved by the implementation
of a Bloom filter (a probabilistic data structure used to efficiently test
whether an element is a member of a set, without any false negative).
Instead of checking every article in the overview database against the
F<history> file to detect orphaned entries, B<expireover> now sequentially
reads the F<history> file at startup to build a Bloom filter, and makes its
checks in that optimized data structure.  On large news spools of billions of
articles, B<expireover> could take several weeks to complete; it now runs in a
few minutes!  This feature is activated by default and controlled by the new
I<expirebloomfp> parameter in F<inn.conf>.  Many thanks to Kevin Bowling for
this improvement.

=item *

The speed of B<expire> has also been drastically improved for large news
spools thanks to the recording of every cancellation alongside the existing
storage and overview deletions, so that B<expire> can just drop matching
history entries without doing per-article retrieval system calls, thus saving
a good amount of time at billion-article scale.  This feature contributed
by Kevin Bowling is not enabled by default.  See the documentation
of I<expiretombstone> in F<inn.conf> for more details, as well as
I<nnrpdcheckart> which also makes use of the cancel tombstone log file.

=item *

Added four new parameters to F<ovsqlite.conf> for the ovsqlite overview
method: I<mmapsize> to have SQLite read data via mmap instead of copying
into its own page cache, I<readercachesize> to set the SQLite in-memory
page cache size for each B<nnrpd> reader process, I<walmode> to enable
SQLite's Write-Ahead Logging journal mode and improve read performance when
multiple B<nnrpd> processes access the overview database simultaneously,
and I<walcheckpointthreshold> to parameterize when to perform checkpoints.
See the ovsqlite(5) manual page for more details.  Thanks to Kevin Bowling for
all these enhancements!

=item *

When I<laxmid> is set in the I<syntaxchecks> parameter in F<inn.conf>,
Message-IDs without any C<@> character are now accepted by B<innd>.
...
Fixed B<makehistory> so that malformed input does not insert a bogus overview
entry.  Thanks to Kevin Bowling for the patch.

=item *

Following upstream rename from blacklist to blocklist, the B<--with-blacklist>
configure flag is renamed B<--with-blocklist> for blocklistd support in
S<FreeBSD 15> and S<NetBSD 10>.  For backward compatibility, both blacklist
and blocklist libraries will be searched by the new B<--with-blocklist> flag.
Thanks to Jose Luis Duran for the information.

=item *

Added support for the ALPN TLS extension in B<nnrpd>.  Thanks to Roman
Donchenko for this feature.

=item *

In consequence of the changes in the libinnstorage(3) interface due to
overview improvements, this library has its SONAME bump in this release.
...
=head1 Changes in 2.7.3 (2025-05-09)

=over 2

=item *

An up-to-date F<moderators> file is provided with this release.  It basically
removes the records of defunct hierarchies.  You may want to manually update
your configuration with the new file.
...
Fixed the handling of empty Message-IDs in B<perl-nocem>.  Instead of dying,
it now just discards these invalid entries.
...
the ovsqlite database.  Thanks to Jesse Rehmer for the bug report.
...
previously couldn't be automatically merged in F<innreport.conf> on update.
...
long having been deprecated, should no longer be sent nor honoured nowadays.
...
authentication, B<nnrpd> was wrongly resetting any knowledge obtained
...
A new "unsigned long" type has been added to the configuration parser.  It
...
(the remainder of a 512-byte blocksize).  On 64-bit systems, the size
...
fewer than S<16 flags>, fixed some bugs and integrated the B<backupfeed>
...
a C<201> return code and also with other codes like C<440> or C<480>.
...
additional authenticators and resolvers.  See libauth(3) for details, and any
...
Various bug fixes, most notably fixes for potential buffer overflow
