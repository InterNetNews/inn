...
The XBATCH command is no longer enabled by default in B<innd>.  You'll have to
explicitly enable that capability by setting the new I<noxbatch> parameter to
false in F<incoming.conf> for the peers sending you such batches.
...
The I<noxbatch> parameter has been added in F<incoming.conf> to enable the
