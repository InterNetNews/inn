...
including TLS and SASL security layers.  As B<innupgrade> only takes
care of the change in the file named F<readers.conf>, you will have
to manually rename that parameter in configuration files for B<nnrpd>
with an alternate name.

=item *

The F<innreport.conf> file in F<pathetc> has been split into a general
...
file to a another name in I<pathlib>, setting this local file name in the new
...
be overwritten each time INN is updated.  Bug fixes or enhancements are made
from time to time to the display configuration of B<innreport>, and couldn't
be automatically be merged in F<innreport.conf> on update.  This new separate
configuration file to parameter the display will now permit an automatic
update (if of course you use the default display configuration file).
...
present in cancel articles was not useful in B<innd>, and relevant to posting
agents.
...
I<nnrppythonauth> parameters in F<inn.conf>, are no longer shipped as
samples in INN releases.  If not already done, you should either replace
old hooks with new modern hooks or use the possibilities F<readers.conf>
and regular authenticator and resolver programs offer.
...
updates, are up-to-date: notably F<control.ctl> (with your local changes
in a separate F<control.ctl.local> file), new better default values in
F<inn.conf> and F<innfeed.conf>, improvements in F<innreport.conf> along
with F<innreport.css>, fixes in F<innwatch.ctl>, updated F<moderators>
and F<nocem.ctl> files.
...
A new B<-F> flag is recognized by B<innconfval> to get the values of that new
configuration file (run C<innconfval -F inn-secrets.conf>).  Another new flag,
B<-f>, permits specifying a file different than the default one.
...
posters have the same identity in an access group.
...
The F<innreport.conf> file in F<pathetc>, previously containing almost 2500
lines, has been split into a general configuration file (F<innreport.conf>
itself, still in F<pathetc>, with about 60 lines) and a display configuration
file (F<innreport-display.conf>, a new separate file in I<pathlib>).
The name of this display configuration file can be parametered in the new
...
B<signcontrol> has been removed as it embeds per-site configuration which is
overwritten each time INN is updated to a newer version, and it is unlikely
you ever need it.  Yet, if you need to issue PGP-signed control messages,
you'll have to download it from L<https://ftp.isc.org/pub/pgpcontrol/>.
