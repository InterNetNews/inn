=head1 Changes in 2.7.2 (not yet released)

=over 2

=item *

An up-to-date F<nocem.ctl> file is provided with this release.  You should
manually update your F<nocem.ctl> file with the new information recorded about
NoCeM issuers, and make sure the right PGP keys are present on your system.
...
Perl or Python filter for B<innd> in a separate storage class, when
...
notably to filter spammy sites.
...
and therefore actually cancel articles before they are locally stored and
spread to other peers.  Thanks to Christoph Biedl for the documentation,
testing, and addition of several useful flags to B<delayer>.
...
as a format like C<Jul 29 04:15:01> was expected.
...
of its configuration file, the keyring to use, and the full B<gpg> and B<gpgv>
commands to run can now be modified by flags passed to the program.  Log
verbosity can be controlled, and B<perl-nocem> can write logs to a separate
...
B<perl-nocem> now correctly parses NoCeM issuers with special characters
like C<+> in the configuration file.  They previously failed to be recognized
...
B<nnrpd> correctly parses header field names with a leading dot in HDR, XHDR
and XPAT commands.

=item *

B<nnrpd> correctly parses header field names with a leading dot (and now
preserves the leading initial dot at injection time).
...
B<inndf> could not count more than 2^32 bytes or inodes).
