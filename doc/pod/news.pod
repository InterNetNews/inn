...
Added a new I<filtered> option in F<storage.conf> to store articles which have
been rejected by a Perl or Python filter for B<innd> in a separate storage
class, when I<dontrejectfiltered> is set to true in F<inn.conf>.  Thanks to
Christoph Biedl for the feature.
...
instance to give cancel control articles and NoCeM notices time to arrive, and
therefore actually cancel articles before they are locally stored and spread
to other peers.  Thanks to Christoph Biedl for the documentation, testing, and
addition of several useful flags to B<delayer>.
