...
with a history manipulation utility, B<hissqlite-util>, that performs some
basic consistency checks and dump operations on an history database using
the hissqlite method, and B<hissqlite-convert> that permits migrating an
existing C<hisv6> history database.  See hissqlite(5) for more details.
New I<hissqlitecachesize>, I<hissqlitemmapsize>, I<hissqlitepagesize> and
I<hissqlitereadercachesize> parameters have also been added to F<inn.conf> to
parameterize this new history storage method.  Many thanks to Keving Bowling
