...
nnrpd: Users can no longer post articles containing Approved: headers to
...
and new OpenPGP signatures and is recommended over PGP 2.6.

=item *

If rc.news.local exists in the INN binary directory, run it whenever
rc.news is run, passing the start or stop argument to it.  This is
