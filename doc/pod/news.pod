...
from a tradspool news spool.

=item *

nnrpd now optionally supports article injection via IHAVE.  Any articles
injected this way must have Date, From, Message-ID, Newsgroups, Path, and
Subject headers.  X-Trace and X-Complaints-To headers will be added if the
appropriate options are set in F<readers.conf>, but other headers will not
be modified/inserted (e.g. NNTP-Posting-Host, NNTP-Posting-Date,
...
innfeed supports a new peer parameter, backlog-feed-first, that if set to
true feeds any backlog to a peer before new articles.  When used in
combination with setting max-connections to 1, this can be used to enforce
in-order delivery of messages to a peer that is doing Xref slaving,
avoiding cases where a higher-numbered message is received before a
lower-numbered message in the same group.
...
For example, see F<authprogs/domain.c>.
