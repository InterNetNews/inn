...
If I<groupbaseexpiry> in inn.conf(5) is true, B<expireover> also removes
old articles from the news spool according to the expiration rules in
expire.ctl(5).  Otherwise it only removes overview entries for articles
...
set in expire.ctl(5), and then when it expires out of the last newsgroup,
...
the backend regardless of expire.ctl(5).  See B<-N>, however.
...
expire.ctl(5) rules say when they're crossposted.  B<-e> and B<-k> cannot
be used at the same time.  This flag is ignored if I<groupbaseexpiry> is
false.

=item B<-N>

Apply control.ctl(5) rules to expire articles even from storage methods
that have self-expire functionality.  This may remove articles from
self-expiring storage methods before the articles expire "naturally."
...
to be expired or not expired without changing expire.ctl(5) for the expire
