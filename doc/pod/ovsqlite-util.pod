...
Naturally, an ovsqlite overview database also needs to be in use, as detailed
...
problems will be reported to standard error.  This is the default action.
...
reliable vacuum on a busy server, stop the news system with C<rc.news stop>
...
churn on every B<expireover> run, and subsequent growth can reuse free pages
...
See also L<ovsqlite(5)/"VACUUMING">.
...
If problems were reported, fix them with:
...
After shrinking retention and letting B<expireover> finish, reclaim freelist
