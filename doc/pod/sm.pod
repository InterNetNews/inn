...
B<grephistory> or stored in the history database).  It looks something like:
...
also appends the cancelled token to F<cancels.tombstone> in I<pathdb>
so that a later B<expire> run can drop the corresponding history entry
...
ctlinnd(8), grephistory(1), hissqlite(5), history(5), rnews(1),
storage.conf(5).
