...
A legal key does not contain blanks, colons, nor C<#>.  There are three
...
a peer's user name retrieved through identd(8) to match the specified string.
...
an empty string, that is to say no password is expected.
