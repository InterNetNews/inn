...
B<inndstart> may log the following messages to syslog (like the rest of
INN, syslog is the best place to find detailed debugging information):
...
=item pathrun (%s) not a directory

(Fatal) I<pathrun> as specified in inn.conf isn't a directory.  This is
where B<innd> will create its local control socket; it should always be a
directory.

=item pathrun (%s) owned by group %d, not %s (%d)

(Fatal) I<pathrun> specified in inn.conf must be owned by the news group.
This is checked for security reasons.

=item pathrun (%s) owned by user %d, not %s (%d)

(Fatal) I<pathrun> specified in inn.conf must be owned by the news user.
This is checked for security reasons.

=item ran by GID %d, who isn't %s (%d)

(Fatal) Someone not in the news group attempted to run B<inndstart>.  If
you were running B<inndstart> as the news user, make sure that the news
user's default group is the news group you picked at compile time (C<news>
by default).

=item ran by UID %d, who isn't %s (%d)
...
=item seteuid(%d) failed: %s

(Fatal) Unable to change the effective UID.  This usually means that
B<inndstart> was installed with the wrong permissions.  If it does have
...
=item setreuid(%d, %d) failed: %s

(Fatal) Unable to change the real and effective UIDs.  This message
usually means that B<inndstart> was installed with the wrong permissions.

=item tried to bind to port %d
...
Passed verbatim from B<inndstart>'s environment.  This is apparently used
on Linux.
...
Read for I<pathnews>, I<pathbin>, I<pathrun>, I<pathtmp>, I<rlimitnofile>,
