...
brackets around I<message-id> are optional.  It should normally be
protected from the shell with single quotes.

I<arrival>, I<expires>, and I<posted> should be the number of seconds
since epoch and indicate when the article arrived, when it expires
(via the Expires header field), and when it was posted (given in the
Date header field), respectively.  I<expires> should be C<0> if the
article doesn't have an Expires header field.  I<token> is the storage
API token for the article, and may be empty.
...
find the specified site and set up a news feed for it.  If the site already
existed, a C<drop> for that site is done first.  This command is
forwarded; see L<"NOTES"> below.

=item cancel I<message-id>

Remove the article with the specified I<message-ID> from the local system.
This does not generate a cancel control message; it only affects the local
system.  The angle brackets around I<message-ID> are optional.  It should
...
when an incoming connection appears to be hung (although B<innd> will close
idle connections automatically after a timeout, as set with I<peertimeout>
in F<inn.conf>).
...
channel number, or for all channels if I<channel> is an empty string.  The
...
The <type> field is one of the following values:
    ...
    exploder    The process for an exploder feed
    ...
    reject      A connection being rejected
...
(a normal, unmoderated group).  The combination of defaults makes it possible
to use the text of the Control header field directly (although don't do this
without checking the syntactic validity of the header field first).

When creating a new group, this command works while the server is
running, paused, or manually throttled; it is rejected only if the
server is throttled because of an I/O error.  (When the newsgroup
already exists, the C<changegroup>-equivalent restrictions apply
instead: the server must be running.)  It will update its internal
state when a C<go> command is sent.  This command updates the
F<active.times> file as well.
...
Enable or disable Perl filtering on every article received by B<innd>.
This command is only available if INN was built with Perl filtering support.
If I<flag> starts with C<y>, filtering is enabled; if it starts with C<n>,
...
Note that enabling or disabling Perl filtering on every post received
by B<nnrpd> is parameterized with the I<perlfilter> access parameter in
F<readers.conf>.

=item python I<flag>

Enable or disable Python filtering on every article received by B<innd>.
This command is only available if INN was built with Python filtering support.
If I<flag> starts with C<y>, filtering is enabled; if it starts with C<n>,
filtering is disabled.
...
The advantage of this parameter over I<noreader> in F<inn.conf> is that it
permits specifying a custom message.
...
reloaded.  (This file is however not available for reloading unless INN
was compiled with Perl filtering support.)  If the Perl filter defined a
function C<filter_before_reload>, it will be called prior to re-reading
F<filter_innd.pl>.  If the Perl function C<filter_after_reload> is defined,
it will be called after F<filter_innd.pl> has been reloaded.  Reloading the
Perl filter does not enable filtering if it has been disabled; use C<perl y>
to do this instead.  F<startup_innd.pl> cannot be reloaded.

If I<what> is the word C<filter.python>, the F<filter_innd.py> file is
reloaded.  (This file is however not available for reloading unless INN
was compiled with Python filtering support.)  If a Python method named
C<filter_before_reload> exists, it will be called prior to re-reading
F<filter_innd.py>.  If a Python method named C<__init__> exists, it will be
called after F<filter_innd.py> has been reloaded.  Reloading the Python filter
does not enable filtering if it has been disabled; use C<python y> to do this.
...
This command works while the server is running, paused, or manually
throttled; it is rejected only if the server is throttled because of an
I/O error.
...
All open connections are closed without any message being sent to them.
It is a good idea to send a C<throttle> command first so that feeds can
be shut down more gracefully.
...
parameter is C<off>, no statistics are gathered.  Otherwise, statistics are
written to the specified file.  A parser for this file is provided in the
contrib tree of the INN distribution.  See I<stathist> in inn.conf(5) for
...
Reject new incoming connections immediately.  Existing incoming
connections are not closed right away; each one is only shut down the
next time it tries to post or transfer an article.  Outgoing feeds are
not affected.  Close the history database.  This should be used for
...
channel, whatever its type (as shown by the C<name> command).  If it
starts with the letter C<i>, general B<innd> tracing is turned on or
off.  If it starts with the letter C<n>, future B<nnrpd> processes
spawned by B<innd> will or will not be passed the B<-t> flag, as
appropriate.  This will not affect any B<nnrpd> processes already
running, or B<nnrpd> processes started by some means other than
B<innd>.
...
to reload configuration files that can't be reloaded via the C<reload>
command.
