...
filtering support in innd may be compiled in by giving C<--with-python>
command line flag to configure script. Python authentication and
authorization may be turned on by nnrppythonauth setting in F<inn.conf>.

If nnrppythonauth is set to true in F<inn.conf>, nnrpd will load the
...
resolved hostname (or IP address if resolution fails) of connected reader
...
IP address of the interface at this machine reader is connected to

=item user

username as reader passed with AUTHINFO command or None if not applicable

=item pass

password as reader passed with AUTHINFO command or None if not applicible
