...
Used in conjunction with delayer(8), B<innfeed> can send a delayed feed to
...
B<innfeed> will catch SIGINT and will write a large debugging snapshot of
...
level starts at zero (unless the B<-d> option is used), in which case no
...
how big the batch file, and no matter how big the I<max-connections>
...
ctlinnd(8), delayer(8), inn.conf(5), innfeed.conf(5), innd(8), procbatch(8).
