...
order of characters is as defined in the UTF-8 character set.  If the
start character of such a range falls after the ending character of the
range in that ranking, the range is malformed and matches nothing beyond
its start character; for example, C<[m-a]> matches only C<m>.
