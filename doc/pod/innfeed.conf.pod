...
The F<innfeed.conf> file configures to which remote peers B<innfeed> sends
NNTP feeds.
...
is either C<true>, C<yes>, C<on>, C<false>, C<no> or C<off> (case is not
...
string needs to contain whitespace, then it must be quoted with double quotes,
and uses the same format for embedding non-printing characters as normal
C-language string.
...
The following listing shows all the keys that apply to the process as a
...
the maximum number of seconds to wait between attempts to reconnect
...
with the newsgroup (e.g. C<bb+%s>).  The default is C<+%s>.
...
specifier which will be replaced with the newsgroup (e.g. C<post+%s@domain>).
...
This key requires a floating-point number which must be in the range
[0.0, 100.0], and it must be smaller than the value for I<no-check-high>.
...
I<no-check-high> value again.  If there is a small difference between this
