...
prunehistory - Remove tokens from Usenet F<history> file
...
This can be useful when B<prunehistory> is used as a filter for other
programs such as B<reap> (a legacy program, probably no longer useful
with current versions of INN, but still available at
L<ftp://ftp.ruhr-uni-bochum.de/.subdisc1/local/jk.collection/>).
