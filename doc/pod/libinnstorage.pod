...
B<ARTcancel> and B<sm -r>).  It is a no-op when I<expiretombstone> is false
in F<inn.conf> or I<token> has type B<TOKEN_EMPTY>.  The function returns true
on success (token appended, flushed, file closed), false on any failure path.
Callers can ignore the return value (the cancel itself has already succeeded
