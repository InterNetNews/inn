...
B<OV> is a common set of utility routines for accessing newsgroups and
overview data independent of particular overview method.

The B<OV> function is to isolate the applications from the individual methods.
All articles passed through the storage API are assumed to be in wire format.
Wire format means C<\r\n> at the end of lines, dot-stuffed lines (that is
to say C<.> at the beginning of lines which already start with C<.>), and
C<.\r\n> at the end of an article on an NNTP stream are not stripped.  This
has a performance win when transferring articles.  Note that for the tradspool
method, wire format can be disabled.  This is just for compatibility which is
...
manager.  I<type> is one of the following:
...
B<SMinit> returns false if any configured storage method fails
initialization.
...
I<amount> is one of the following, which specifies the retrieval type:
...
be cancelled by B<SMcancel>.  The data area indicated by B<ARTHANDLE>
...
The B<SMcanceltombstone> function appends the I<token> to the out-of-band
cancel tombstone log so a later expire run can drop the matching history
entry without calling B<SMretrieve>.  It is intended for callers that
cancel articles outside the B<expireover>/B<expirerm> pipeline (B<innd>'s
B<ARTcancel> and B<sm -r>).  It is a no-op when I<expiretombstone> or
I<groupbaseexpiry> is false in F<inn.conf>, or when I<token> has type
B<TOKEN_EMPTY>.  The function returns true on success (token appended,
flushed, file closed), false on any failure path.  Callers can ignore
the return value (the cancel itself has already succeeded by the time
this is called).
...
method.  I<type> is one of the following:
...
as I<ovmethod> in F<inn.conf>.  I<mode> is constructed from the following:
...
Pass the Bloom filter to B<OVhisthasmsgid>.
...
The B<OVadd> function stores overview data.
...
data, or arrival time for the next article in the range opened by
B<OVopensearch>.  It should be called with the I<handle> returned by
B<OVopensearch>, and that same I<handle> should be passed to every
subsequent call to B<OVsearch> until it returns false.  B<OVsearch> returns
