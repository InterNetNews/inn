...
by a newline, will be output at the start of the first batch written by
this invocation of B<batcher>, but not at the start of any further batch
even if options such as B<-a>, B<-b>, or B<-N> cause more than one batch
to be created.  The default is to have no initial string (under the
assumption that either the processor specified with the B<-p> flag or
