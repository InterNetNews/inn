...
will need to be upgraded using ovdb_init(8).  See the man page ovdb_init(8)
...
to have subdirectories F<include> and F<lib>, containing
F<db.h>, and the library itself, respectively.
...
The ovdb database is stored in the directory specified by the I<pathoverview>
paramter in F<inn.conf>.  This is the "DB_HOME" directory.  To start out,
this directory should be empty (other than an optional F<DB_CONFIG> file,
...
Other parameters for configuring ovdb are in the optional F<ovdb.conf>
...
in the F<DB_CONFIG> file.  If the pathname you give starts with a /,
...
exist and have proper ownership/mode before starting INN, because 
they won't be created automatically.  Also, don't change the DB_CONFIG
...
When starting the news system, C<rc.news> will invoke C<ovdb_init>.
C<ovdb_init> must be run before using the database.  It performs
the following tasks:
...
And when stopping INN, F<rc.news> kills the ovdb_monitor processes after the
...
C<kill -9> if necessary) and then restart.  C<ovdb_init> should
...
inn.conf(5), ovdb_init(8), ovdb_monitor(8), ovdb_stat(8)
