...
In general, the bigger the cache, the better.  Use C<db_stat -m> to see
...
You don't need to do anything special when starting or stopping INN
when using ovdb, as long as you use F<rc.news>.  When starting,
F<rc.news>:
...
Runs C<dbprocs start>.  This starts the DB housekeeping processes.

=back

And when stopping INN, F<rc.news> calls C<dbprocs stop> after the
...
bin dir to PATH and set DB_HOME to innconf->pathoverview.  Since I
don't know TCL, someone needs to modify innshellvars.tcl in the same
way.
