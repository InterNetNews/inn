...
other type of INN overview.  (See dedicated ovdb_stat(8) and ovsqlite-util(8)
programs for ovdb and ovqslite overview methods.)  It allows the administrator
to dump various information about the internal state of the overview, audit it
...
Only useful in combination with the B<-o> and B<-O> options to dump overview
...
Specify the newsgroup on which to act, required for the B<-c>, B<-g>, B<-o>,
B<-O>, and B<-R> options, and optional for the B<-i> option.
...
(from the Expires header field) in S<RFC 5322> date format if there is any,
labelled with C<Expires:>.
...
same format (or 0 if there is no Expires header field), and then the overview
data.
