...
B<nnrpd> [B<-DfnoStB>] [B<-4> I<address>] [B<-6> I<address>] [B<-b> I<address>]
...
If specified, B<nnrpd> will report login attempts to blacklistd for
...
B<--with-blacklist> specified at configure time.
...
blacklistd is a FreeBSD daemon for preventing brute force attacks by
blocking attackers after a number of failed login attempts. When
nnrpd is built with blacklist support, it will report login attempts
...
See the blacklistd documentation for more information.
...
ctlinnd(8), blacklistd(8), innd(8), inn.conf(5), libinn_uwildmat(3),
nnrpd.track(5), passwd.nntp(5), readers.conf(5), signal(2).
