...
fields of a history database entry, the I<arrival> is normally the time at
...
article, and the I<paths> field is the storage API token.)  Returns true
...
for people familiar with the old mechanism (identifiable by the
nnrpperlauth: parameter in F<inn.conf>).  Other people should skip this
section.

The perl_auth parameter allows the use of perl to authenticate a user.
It works in the same manner as the auth parameter (part of
readers.conf):
...
a perl script.  If a connection matches an auth group with a
perl_access parameter, all access groups in readers.conf are ignored
and the procedure described below is used to generate an access group.
...
specification) and supply a simple readers.conf file.
...
functionality is not available directly, you may prefer writing a new
authenticator or resolver in any language to working in perl.
