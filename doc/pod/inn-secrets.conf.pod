...
news software implements SHA-256), and the other hash uses the mandatory
...
algorithms (one is enough) and verify that the resulting Base64-encoded hash is
...
The news administrator is therefore capable of sending authenticated withdrawal
