...
not make any changes to the F<active> file, and write debug messages to
standard error.  Note that using I<debug-level> is only supported when
synchronizing with another news server, not with FTP or HTTP(S).

Unless the I<debug-format> argument is also given to B<actsyncd>, the data
written to standard output will be in the format determined by the C<flags=>
line, that is to say C<-o c> (B<ctlinnd> commands) since you should not
specify an B<-o> flag on that line.  If I<debug-format> is given, the data
written to standard output will be in C<-o I<debug-format>> format instead.
...
The newsgroups directory of a newsgroup with an all numeric component
...
be considered for removal.  This causes B<actsync> to simply ignore such
