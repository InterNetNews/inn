...
B<inndf> [B<-hi>] I<directory> [I<directory> ...]

B<inndf> [B<-i>] [B<-f> I<filename>] [B<-F>] I<directory> [I<directory> ...]
...
If I<enableoverview> set set to true in F<inn.conf>, B<inndf> can also be
...
I<filename> should contain a list of directories to use in addition to those
given by the arguments. Syntax of this file is that blank lines are ignored
and everything after a '#' is ignored.

=item B<-F>

B<-F> is like B<-f> execpt that the filename is I<pathetc>/inndf.conf.
