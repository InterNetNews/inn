...
Setting it to C<never> means history records will be retained forever, C<0>
means history records for articles not present in the spool will be purged
when expiry programs run.  (This is notably useful in case you need to
re-inject articles no longer present in your news spool but considered
duplicates because they are still present in history: you'll have to set
C</remember/>
to C<0>, run the B<expire> process for instance via B<news.daily> called with
the same parameters as in crontab plus C<notdaily>, and undo the change in
F<expire.ctl>.  All previously rejected or removed articles will then not be
considered as duplicate if their Message-ID is proposed.)
...
There is no way to set an expiration policy for articles crossposted to
groups you don't carry that's different than other articles in the same group.
...
where <flag> contains C<X>, it is deleted out of all newsgroups to which it
was posted immediately.  (If that behaviour is wanted for all rules, you may
want to give the B<-e> flag to B<expireover>.)
...
header field.  Articles with an Expires header field will be expired
at the date given in that header field, subject to the constraints that
they will be retained at least <min> days and no longer than <max> days.
...
without an explicit Expires header field will ever be expired.  If <max> is
set to C<never>, Expires header fields will be honoured no matter how far into
the future they are.
...
It is often useful to honour the Expires header field in articles, especially
...
When I<groupbaseexpiry> is false, for class-based expiration:
    ...
    *:1:7:35
    ...
    # Class 0 is retained for two weeks and honour Expires header fields
