...
The F<readers.conf> file determines who is allowed to connect as a news
...
if a domain is specified (like C<< <username>@<domain> >>) or otherwise just
C<< <username> >>.
...
Auth and access blocks can also be wrapped in a C<group> block so that
common parameter settings do not have to be repeated in every one of
them:

    group <name> {
        <parameter>: <value>
        # ...possibly other settings shared by the blocks below
...
As with auth and access blocks, the <name> of a group is only for
documentation purposes.  Any parameter set directly in a group block is
used as the starting value for every auth or access block (and any
nested group) declared inside it; each of those blocks may still
override that value with a setting of its own.  Groups may themselves be
nested inside other groups, in which case they inherit the enclosing
group's settings the same way.  This is purely a convenience to avoid
repeating identical parameter values across several auth or access
blocks; it has no other effect on how auth and access blocks are
matched.
...
the maximum line length of 8,191 characters.  In that case, you'll need to
...
in the I<pathfilter> directory will be used to determine the client's rights
...
F<inn.conf>) for the domain part of the newsmaster's e-mail address and the
complaints address in the Injection-Info header field, if its default value
should be changed, and the I<complaints> parameter is not already set or
I<newsmaster> does not already contain an C<@>.

=item I<groupexactcount>

This parameter controls the threshold below which B<nnrpd> should give the
...
expiry process by B<expireover> (run by B<news.daily>), and is updated
when new articles arrive but not necessarily when articles are cancelled or
...
Used as the contact address in the help message returned by B<nnrpd>.  If
unset, the address will consist of the newsmaster name selected at configure
time (C<usenet> by default), an C<@>, and the value of I<fromhost> as set in
F<inn.conf> or overridden in F<readers.conf>.

If this parameter is set and does not contain C<@>, the contact address will
consist of the given value followed by an C<@> and the value of I<fromhost>.
...
You may also want to set the I<complaints>, I<domain>, I<fromhost>, and
I<newsmaster> parameters to specific values if you use this parameter.

Note that setting this parameter requires the server to modify all posts before
...
an explicit I<users> key so as to better see to whom the access block
applies.
...
line needs to be first, with the C<< users: <EXAMPLE> >> group after.
...
For more complex cases, or if you just prefer to use a database to store
authentication and/or authorization information, you can use Perl or Python
scripts to dynamically authenticate the users and/or generate their access
blocks.  You may then put all the related information (user, password,
newsgroups for reading, newsgroups for posting, etc.) for instance in an
SQLite database, and use an F<nnrpd_auth.pl> or F<nnrpd_auth.py> script to
authenticate the users, and/or an F<nnrpd_access.pl> or F<nnrpd_access.py>
script to grant the access.  Then F<readers.conf> only needs defining:
...
The advantage of these external scripts is that you can code whatever rules
you want, possibly based on a database shared between your news service and
other services you may provide to your users.  You can more easily handle
hundreds or thousands of users with external scripts than with the mere use
of F<readers.conf> stock blocks and a flat file for the credentials of each
listed user.
