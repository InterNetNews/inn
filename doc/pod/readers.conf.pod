...
As for authorization, let's do something for people successfully authenticated
with passwords:
...
F<readers.conf> before C<restrictive> or C<readonly> blocks because
authorization blocks are checked from the last one in the F<readers.conf>
file to the first one.  As soon as one matches, it is chosen for the identity
defined in an authentication block.  The C<authenticatedpeople> access block
matches every user that has not been assigned C<< <unauthenticated> >> as
identity.
...
set this parameter to true, you must also set either I<pathhost> or I<domain>
...
their Path and Xref header field bodies altered to appear to be from the server
named in I<pathhost> or I<domain> (whichever is set), and posted articles will
use that server name in the Path, Message-ID, and Injection-Info header
field bodies.
