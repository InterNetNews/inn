...
system that way.  As several different persons can be assigned the same
"<SPECIAL>" identity, no user-specific Cancel-Lock hashes should be generated.
...
If set to false, posts made by these users will not have a user-specific
...
reason for this parameter is to deactivate this authentication mechanism when
the same identity can be assigned by an access group to different people.
