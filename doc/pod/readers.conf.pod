...
successful auth group contains the perl_access parameter, F<readers.conf>
...
should be changed, and the I<newsmaster> and I<complaints> parameters do not
already contain a C<@>.

=item B<groupexactcount:>
...
in self-expiry storage methods like CNFS.
...
time (C<usenet> by default), a C<@>, and the value of I<fromhost> as set in
F<inn.conf> or overridden in F<readers.conf>.

If this parameter is set and does not contain C<@>, the contact address will
consist of the given value followed by a C<@> and the value of I<fromhost>.
...
running on a server with a different name than it actually is.  The I<domain>
...
descriptions of these parameters:
    ...
    backoff_trigger, checkincludedtext, clienttimeout, complaints,
