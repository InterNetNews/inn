...
up.  (The location I<pathetc>/readers.conf is only the default; the same
format applies to any file specified with C<nnrpd -c>.)
...
that user ispermitted to do.  Writing a F<readers.conf> file for your
...
appropriate privileges with access group.  We recommend I<not>
intermingling auth groups and access groups in the config file; it is
often more sensible (in the absence of the I<key> parameter) to put all
...
mechanism such as AUTHINFO USER/PASS.  Like with res:, one auth group
...
access group with a newsgroups: parameter.
...
in the same access group if this parameter is set to true.  nnrpd(8) will
act as if the server name is the value of pathhost:, or domain: if
pathhost: isn't set or is set to the same value as in F<inn.conf>.  One of
...
provide a username and password, which is then checked against RADIUS.
...
can use the ssl_required: parameter, or C<-c> to specify an alternate
