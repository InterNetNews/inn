...
it is encrypted using SSL.  This parameter is only valid given at configure
time with B<--with-openssl>.
...
the client cannot post.  And if this parameter is given, I<allownewnews> in
inn.conf(5) is never used to determine permission of NEWNEWS command usage.
