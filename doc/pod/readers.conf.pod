...
I<pathbin>/auth/resolv.  A resolver is an authentication program which
...
Note that if this parameter is given, I<allownewnews> in inn.conf(5) is
...
All auth groups are scanned and the ones that don't match the client IP
