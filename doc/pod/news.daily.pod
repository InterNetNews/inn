...
Using this keyword disables the rotating and cleaning aspect of the
log processing: the logs files are only scanned for information and
no contents are altered.  This keyword has no effect if the B<nologs>
keyword is used.  The B<norotate> keyword is passed on to B<scanlogs>
if it is invoked.
