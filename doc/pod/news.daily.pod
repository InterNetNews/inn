...
purging the overview database with B<expireover> (if the corresponding
eponymous keyword is specified or if I<enableoverview> is true in
F<inn.conf>);
...
rebuilding the history database with B<expire>;
...
By default, B<expire> builds the new history database in the same directory as
the current one.  Using this keyword specifies a different location to build
the new C<hisv6> database (by passing the B<-d> flag to B<expire>), which will
...
F<active>.  Normal C<ctlinnd renumber> operation will take a long time.  With
...
fastrm(1), hissqlite(5), history(5), inn.conf(5), innstat(8), newslog(5),
procbatch(8), scanlogs(8), shlock(8).
