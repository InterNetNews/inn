...
If I<ovmethod> in F<inn.conf> is C<ovdb>, you should have the ovdb
processes running while rebuilding overview; otherwise, the Berkeley DB
transaction logs will be extremely large by the time makehistory finishes.
You can start them with ovdb_init(8).
