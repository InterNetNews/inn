...
DB transaction logs will be extremely large by the time makehistory
finishes.  You can start them with dbprocs(8).
...
only if B<-O> flag is specified and overview.fmt includes C<Bytes:>.
...
dbz(3), active(5), history(5), inn.conf(5), dbprocs(8), ctlinnd(8),
innd(8), makedbz(8).
