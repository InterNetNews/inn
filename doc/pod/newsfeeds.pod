...
Path header field body entry unintentionally.  For this reason, special
local entries (such as archivers or gateways) should probably end with
an exclamation point to make sure that they do not have the same name
as any real site.  For example, C<gateway> is an obvious name for the
local entry that forwards articles out to a mailing list.  If a site with
the name C<gateway> posts an article, when the local site receives the
article it will see the name in the Path and not send the article to its
own C<gateway> entry.  Since C<gateway!> can't appear as an individual
Path header field body entry since C<!> is a delimiter in the Path
header field body, that would be a better thing to use for I<sitename>.
...
(It's sometimes convenient to have the I<sitename> be an abbreviated form
of the name of the remote site, since all the I<sitename>s to which an
article is sent are written to the log and using shorter I<sitename>s
can therefore improve performance for large servers.  In this case,
the Path header field body entries of those sites should be given as
I<exclude> entries and the C<Ap> flag used so that the abbreviated
I<sitename> doesn't accidentally match some other Path header field
body entry.)
...
with a particuliar distribution.
...
same as the Path header field body entries added by those remote sites,
or for program feeds where the site name is arbitrary and unrelated to
...
name in the header, and some sites that might logically be considered
to be one hop become two because they put the posting workstation's name
in the header.  The default value for I<count> if not specified is one.
(Also see the B<O> flag, which is sometimes more appropriate for some
uses of this flag.)
...
uwildmat(3) expression without commas or a list of such expressions,
...
fed from localhost by a program like rnews(8)), the Path header field body
