...
wildmat(3) pattern supporting poison (C<@>) wildcards; see the wildmat(3)
...
I<dontrejectfiltered> is set in inn.conf(5).  With that variable set, this
...
I<originator>.  I<originator> is a wildmat(3) expression without commas or
...
The storage API token of the article (the same as C<n>).
...
value of I<logipaddr> in inn.conf(5).  If I<logipaddr> is true and the IP
...
innfeed(8), innxmit(8), nntpsend(8), wildmat(3).
