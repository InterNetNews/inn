...
B<inews> [B<-ADhNORsSVW>] [B<-acdeFfnortwx> I<value>] [B<-p> I<port>] [I<file>]

=head1 DESCRIPTION

B<inews> reads a Usenet news article, perhaps with headers, from I<file>
or standard input if no file is given.  It adds some header fields and performs
...
If not provided, the Path header field of an article is constructed as follows:
The basic Path header field body will be "not-for-mail".  If I<pathhost> is specified
...
header field body; it will not replace it.
...
B<inews> and the hostname of the news server.
