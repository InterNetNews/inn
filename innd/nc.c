...
static NCDISPATCH NCcommands[] = {
    ...
    COMMAND("CAPABILITIES",  NCcapabilities, false, 1, 2, true,  "[keyword]"),
    COMMAND("CHECK",         NCcheck,        true,  2, 2, false, "message-ID"),
    COMMAND("HEAD",          NChead,         true,  1, 2, true,  "message-ID"),
    COMMAND("HELP",          NChelp,         false, 1, 1, true,  NULL),
    COMMAND("IHAVE",         NCihave,        true,  2, 2, true,  "message-ID"),
    COMMAND("LIST",          NClist,         true,  1, 3, true,
        "[ACTIVE [wildmat]|ACTIVE.TIMES [wildmat]|MOTD|NEWSGROUPS [wildmat]]"),
    COMMAND("MODE",          NCmode,         false, 2, 2, true,  "READER"),
    COMMAND("QUIT",          NCquit,         false, 1, 1, true,  NULL),
    COMMAND("STAT",          NCstat,         true,  1, 2, true,  "message-ID"),
    COMMAND("TAKETHIS",      NCtakethis,     true,  2, 2, false, "message-ID"),
    COMMAND("XBATCH",        NCxbatch,       true,  2, 2, true,  "size"),
...
NCpostit(CHANNEL *cp)
{
    ...
    if (Mode == OMthrottled) {
        ...
        cp->Reported++;
        if (cp->Sendid.size > 3) {
            /* In streaming mode, there is no NNTP_FAIL_TAKETHIS_DEFER and RFC
             * 4644 mentions that we MUST send 400 here and close the
             * connection so as not to reject the article. Yet, we could have
             * sent NNTP_FAIL_ACTION without closing the connection... */
            cp->State = CSwritegoodbye;
            snprintf(buff, sizeof(buff), "%d %s", NNTP_FAIL_TERMINATING,
                     ModeReason);
        } else {
            cp->State = CSgetcmd;
            snprintf(buff, sizeof(buff), "%d %s", NNTP_FAIL_IHAVE_DEFER,
                     ModeReason);
        }
    ...
    if (ARTpost(cp)) {
        ...
        if (cp->Sendid.size > 3) { /* We are streaming. */
            ...
            snprintf(buff, sizeof(buff), "%d Article transferred OK",
                     NNTP_OK_IHAVE);
            response = buff;
        }
    } else {
...
NCxbatch(CHANNEL *cp)
{
    ...
    if (cp->XBatchSize <= 0
        ...
        syslog(L_NOTICE, "%s got bad xbatch size %d", CHANname(cp),
               cp->XBatchSize);
...
NClist(CHANNEL *cp)
{
    ...
    if (cp->ac == 1 || (strcasecmp(cp->av[1], "ACTIVE") == 0)) {
        ...
        if (qp == NULL) {
            ...
            xasprintf(
                &buff,
                "%d Newsgroup descriptions in form \"group description\"",
                NNTP_OK_LIST);
            NCwritereply(cp, buff);
            free(buff);
        }
        ...
        if (qp == NULL) {
            xasprintf(&buff,
                      "%d No list of newsgroup creation times available",
                      NNTP_ERR_UNAVAILABLE);
            NCwritereply(cp, buff);
            free(buff);
            return;
            ...
            xasprintf(&buff,
                      "%d Newsgroup creation times in form \"group time who\"",
                      NNTP_OK_LIST);
            NCwritereply(cp, buff);
            free(buff);
        }
    ...
    while ((p = QIOread(qp)) != NULL) {
        ...
        if (checkutf8) {
            if (!is_valid_utf8(p)) {
                syslog(L_ERROR,
                       "%s NClist bad encoding in %s (UTF-8 expected)",
                       CHANname(cp), cp->av[1]);
                continue;
            }
        }
...
NCmode(CHANNEL *cp)
{
    ...
    if (strcasecmp(cp->av[1], "READER") == 0 && !innconf->noreader) {
        /* MODE READER. */
        syslog(L_NOTICE, "%s NCmode \"MODE READER\" received", CHANname(cp));
        if (!cp->CanAuthenticate) {
            /* AUTHINFO has already been successfully used. */
            snprintf(buff, sizeof(buff),
                     "%d Already authenticated as a feeder", NNTP_ERR_ACCESS);
            NCwritereply(cp, buff);
            return;
        }
        if (NNRPReason != NULL && !innconf->readerswhenstopped) {
            /* Server paused or throttled. */
            snprintf(buff, sizeof(buff), "%d %s", NNTP_FAIL_ACTION,
                     NNRPReason);
            NCwritereply(cp, buff);
            return;
        } else {
        ...
        NCwritereply(cp, buff);
        syslog(L_NOTICE, "%s NCmode \"MODE STREAM\" received", CHANname(cp));
        return;
    } else if (strcasecmp(cp->av[1], "CANCEL") == 0 && cp->privileged) {
        /* MODE CANCEL */
        cp->State = CScancel;
        snprintf(buff, sizeof(buff), "%d Cancels permitted",
                 NNTP_OK_MODE_CANCEL);
        NCwritereply(cp, buff);
        syslog(L_NOTICE, "%s NCmode \"MODE CANCEL\" received", CHANname(cp));
        return;
    } else {
        /* Unknown MODE command or readers not allowed. */
        snprintf(buff, sizeof(buff), "%d Unknown MODE variant",
                 NNTP_ERR_SYNTAX);
        NCwritereply(cp, buff);
...
NC_reader(CHANNEL *cp)
{
    ...
    if (!cp->CanAuthenticate || innconf->noreader
        || (NNRPReason != NULL && !innconf->readerswhenstopped))
        snprintf(buff, sizeof(buff), "%d Permission denied", NNTP_ERR_ACCESS);
    else
        snprintf(buff, sizeof(buff), "%d MODE-READER", NNTP_FAIL_WRONG_MODE);
    NCwritereply(cp, buff);
}
...
NCproc(CHANNEL *cp)
{
    ...
    NCDISPATCH *dp;
    struct buffer *bp;
    char buff[NNTP_MAXLEN_COMMAND]; /* For our (long) answers for
                                     * CHECK/TAKETHIS, we need at least the
                                     * length of the command (512 bytes). */
    size_t i, j;
    ...
    for (;;) {
        ...
        case CScancel:
            ...
            if (i - cp->Start > NNTP_MAXLEN_COMMAND) {
                for (dp = NCcommands; dp < ARRAY_END(NCcommands); dp++) {
                    if ((dp->Function != NC_unimp)
                        ...
                        if ((!StreamingOff && cp->Streaming)
                            || (dp->Function != NCcheck
                                && dp->Function != NCtakethis)) {
                            validcommandtoolong = true;
                        }
                        /* Return 435/438 instead of 501 to IHAVE/CHECK
                         * commands for compatibility reasons. */
                ...
                if (strcasecmp(cp->av[0], "TAKETHIS") != 0
                    ...
                    if (syntaxerrorcode == NNTP_FAIL_CHECK_REFUSE) {
                        ...
                        snprintf(buff, sizeof(buff), "%d Line too long",
                                 validcommandtoolong ? syntaxerrorcode
                                                     : NNTP_ERR_COMMAND);
                    }
                    NCwritereply(cp, buff);
                    cp->Start = cp->Next;
            ...
            if (dp == ARRAY_END(NCcommands)) {
                if (++(cp->BadCommands) >= BAD_COMMAND_COUNT) {
                    cp->State = CSwritegoodbye;
                    snprintf(buff, sizeof(buff),
                             "%d Too many unrecognized commands",
                             NNTP_FAIL_TERMINATING);
                    NCwritereply(cp, buff);
                    break;
                }
                if (strcasecmp(cp->av[0], "XYZZY") == 0) {
                    /* Acknowledge the magic word from the Colossal Cave
                ...
                for (v = cp->av; *v; v++)
                    if (strlen(*v) > NNTP_MAXLEN_ARG) {
                        ...
                        if (syntaxerrorcode == NNTP_FAIL_CHECK_REFUSE) {
                            ...
                            snprintf(buff, sizeof(buff),
                                     "%d Argument too long", syntaxerrorcode);
                        }
                        break;
                    }
                if (validcommandtoolong) {
                    /* If TAKETHIS, we have to read the entire multi-line
                     * response block before answering. */
                    if (strcasecmp(cp->av[0], "TAKETHIS") != 0
                        || (StreamingOff || !cp->Streaming)) {
                        NCwritereply(cp, buff);
                        cp->Start = cp->Next;
            ...
            if ((dp->Minac != NC_any && cp->ac < dp->Minac)
                ...
                if (syntaxerrorcode == NNTP_FAIL_CHECK_REFUSE) {
                    ...
                    snprintf(buff, sizeof(buff), "%d Syntax is: %s %s",
                             syntaxerrorcode, dp->Name,
                             dp->Help ? dp->Help : "(no argument allowed)");
                }
                /* If TAKETHIS, we have to read the entire multi-line response
                 * block before answering. */
                if (strcasecmp(cp->av[0], "TAKETHIS") != 0
            ...
            if (dp->Needauth && !cp->IsAuthenticated) {
                if (cp->CanAuthenticate) {
                    ...
                    snprintf(buff, sizeof(buff), "%d Access denied",
                             NNTP_ERR_ACCESS);
                }
                /* If TAKETHIS, we have to read the entire multi-line response
                 * block before answering. */
        ...
        fallthroughCSgotarticle:
            ...
            movedata = false;
            if (Mode == OMthrottled) {
                /* We do not remember the message-ID of this article because it
                 * has not been stored. */
                ARTreject(REJECT_OTHER, cp);
        ...
        case CSeatcommand:
            ...
            if (i < bp->used) { /* did find terminator */
                ...
                syslog(
                    L_NOTICE,
                    "%s internal rejecting too long command line (%lu > %d)",
                    CHANname(cp), (unsigned long) i, NNTP_MAXLEN_COMMAND);
                cp->LargeCmdSize = 0;
                /* Command eaten; we do not know whether it was valid (500 or
                 * 501). */
                snprintf(buff, sizeof(buff),
                         "%d command exceeds limit of %d bytes",
        ...
        case CSgetxbatch:
             ...
             */
            if (Tracing || cp->Tracing)
                syslog(L_TRACE, "%s CSgetxbatch: now %lu of %d bytes",
                       CHANname(cp), (unsigned long) bp->used, cp->XBatchSize);
            ...
            /* now do something with the batch */
            {
                ...
                if (fd < 0) {
                    ...
                    syslog(L_ERROR, "%s cannot open outfile %s for xbatch: %m",
                           CHANname(cp), buff);
                    snprintf(buff, sizeof(buff),
                             "%d XBATCH failed -- cant create file: %s",
                             NNTP_FAIL_XBATCH, strerror(oerrno));
                    NCwritereply(cp, buff);
                } else {
                    if (write(fd, cp->In.data, cp->XBatchSize)
                        != cp->XBatchSize) {
                        oerrno = errno;
                        ...
                        snprintf(
                            buff, sizeof(buff),
                            "%d XBATCH failed -- cant write batch to file: %s",
                            NNTP_FAIL_XBATCH, strerror(oerrno));
                        NCwritereply(cp, buff);
                        failed = 1;
                    }
                ...
                if (fd >= 0 && close(fd) != 0) {
                    ...
                    snprintf(
                        buff, sizeof(buff),
                        "%d XBATCH failed -- error closing batch file: %s",
                        NNTP_FAIL_XBATCH, strerror(oerrno));
                    NCwritereply(cp, buff);
                    failed = 1;
                }
                ...
                if (rename(buff, buff2)) {
                    ...
                    snprintf(buff, sizeof(buff),
                             "%d XBATCH failed -- cant rename batch to %s: %s",
...
NCreader(CHANNEL *cp)
{
    ...
    if (Tracing || cp->Tracing)
        syslog(L_TRACE, "%s NCreader Used=%lu", CHANname(cp),
               (unsigned long) cp->In.used);
...
NCsetup(void)
{
    ...
    if (p == NULL)
        ...
        p = Path.data;
    snprintf(buff, sizeof(buff),
             "%d %s InterNetNews server %s ready (transit mode)",
...
NCcreate(int fd, bool MustAuthorize, bool IsLocal)
{
    ...
    if (!IsLocal) {
        ...
        i = 1;
        if (setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (char *) &i, sizeof i)
            < 0)
...
NCcheck(CHANNEL *cp)
{
    ...
    if (!IsValidMessageID(cp->av[1], false, laxmid)) {
        snprintf(cp->Sendid.data, cp->Sendid.size,
                 "%d %s Syntax error in message-ID", NNTP_FAIL_CHECK_REFUSE,
                 cp->av[1]);
        NCwritereply(cp, cp->Sendid.data);
        syslog(L_NOTICE, "%s bad_messageid %s", CHANname(cp),
               MaxLength(cp->av[1], cp->av[1]));
...
NCtakethis(CHANNEL *cp)
{
    ...
    if (!cp->IsAuthenticated) {
        returncode =
            cp->CanAuthenticate ? NNTP_FAIL_AUTH_NEEDED : NNTP_ERR_ACCESS;
    }
...
#if defined(DO_PERL) || defined(DO_PYTHON)
    if (filterrc != NULL) {
        snprintf(cp->Sendid.data, cp->Sendid.size, "%d %s %.200s", returncode,
                 mid, filterrc);
    } else {
        snprintf(cp->Sendid.data, cp->Sendid.size, "%d %s", returncode, mid);
    }
#else
    snprintf(cp->Sendid.data, cp->Sendid.size, "%d %s", returncode, mid);
#endif /* defined(DO_PERL) || defined(DO_PYTHON) */
