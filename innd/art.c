...
ARTlog(const ARTDATA *data, char code, const char *text)
{
    ...
    t = Now.tv_sec;
    if (text)
        i = fprintf(Log, "%.15s.%03d %c %s %s %s%s", ctime(&t) + 4,
                    (int) (Now.tv_usec / 1000), code,
                    data->Feedsite != NULL ? data->Feedsite : "(null)",
                    HDR_FOUND(HDR__MESSAGE_ID) ? HDR(HDR__MESSAGE_ID)
                                               : "(null)",
                    text, Done ? "" : "\n");
    else
        i = fprintf(Log, "%.15s.%03d %c %s %s%s", ctime(&t) + 4,
                    (int) (Now.tv_usec / 1000), code,
                    data->Feedsite != NULL ? data->Feedsite : "(null)",
                    HDR_FOUND(HDR__MESSAGE_ID) ? HDR(HDR__MESSAGE_ID)
                                               : "(null)",
...
ARTparsepath(const char *p, int size, LISTBUFFER *list)
{
    ...
    for (i = 0, q = list->Data, hp = list->List; *p; p++, *q++ = '\0') {
        ...
        if (list->ListLength <= i) {
            ...
            list->List =
                xrealloc(list->List, list->ListLength * sizeof(char *));
            hp = &list->List[i];
        }
...
ARTstore(CHANNEL *cp)
{
    ...
    for (data->BytesValue = i = 0; i < iovcnt; i++) {
        if (NeedHeaders && (i + 1 == iovcnt)) {
            ...
            headersize = data->BytesValue + Article->data + data->Body
                         - (char *) iov[i].iov_base;
        }
...
ARTcheckheader(CHANNEL *cp, int size)
{
    ...
    *colon = ':';
    ...
    if (hc->Length != 0) {
         ...
         * We do not check every header field because they would otherwise
         * disappear from the overview. The content of the first occurrence
         * must be returned by HDR and OVER according to RFC 3977. */
        ...
        if (i == HDR__MESSAGE_ID || i == HDR__SUPERSEDES
            ...
            for (p = colon + 1; (p < header + size - 2) && (ISWHITE(*p)); p++)
                ;
            hc->Value = p;
            for (p = header + size - 3; (p > hc->Value) && (ISWHITE(*p)); p--)
                ;
...
ARTprepare(CHANNEL *cp)
{
    ...
    data->Lines = data->HeaderLines = data->CRwithoutLF = data->LFwithoutCR =
        0;
...
ARTassignnumbers(ARTDATA *data)
{
    ...
    for (linelen = i = 0; (ngp = GroupPointers[i]) != NULL; i++) {
        ...
        if (len + 1 + ngp->NameLength + 1 + ARTNUMPRINTSIZE + 2 + 2
            > data->XrefBufLength) {
        ...
        if (linelen + 1 + ngp->NameLength + 1 + ARTNUMPRINTSIZE + 2
            > MAXARTLINELENGTH) {
            /* Line exceeded. */
    ...
    /* p[0] is replaced with '\r' to be wireformatted when stored.  p[1] needs
...
ARTxrefslave(ARTDATA *data)
{
    ...
    for (i = 0; (*p != '\0') && (p < HDR(HDR__XREF) + HDR_LEN(HDR__XREF));
         p = next) {
        ...
        *p = ':';
        ...
        if (ngp->Last < ngp->Filenum) {
            ...
            if (!FormatLong(ngp->LastString, (long) ngp->Last,
                            ngp->Lastwidth)) {
                syslog(L_ERROR, "%s cant update_active %s", LogName,
                       ngp->Name);
                continue;
            }
        }
...
HashFeedMatch(HASHFEEDLIST *hf, char *MessageID)
{
    ...
    while (hf) {
        ...
        if ((h % hf->mod + 1) >= hf->begin && (h % hf->mod + 1) <= hf->end)
            return true;
        hf = hf->next;
    }
             ...
             bool ControlStore, bool OverviewCreated, bool Filtered)
{
        ...
        if (sp->Originator) {
            if (!HDR_FOUND(HDR__XTRACE) && !HDR_FOUND(HDR__INJECTION_INFO)) {
                ...
                *p = '\0';
                ...
                for (j = 0, sendit = false; (q = sp->Originator[j]) != NULL;
                     j++) {
                    if (*q == '@') {
                ...
                *p = savec;
                ...
                if ((p = strchr(HDR(HDR__XTRACE), ' ')) != NULL) {
                    *p = '\0';
                    for (j = 0, sendit = false;
                         (q = sp->Originator[j]) != NULL; j++) {
                        if (*q == '@') {
    ...
    if (putc('\n', Log) == EOF || (!BufferedLogs && fflush(Log))
        || ferror(Log)) {
...
ARTmakeoverview(CHANNEL *cp)
{
    ...
    for (fp = ARTfields; fp->Header; fp++) {
        ...
        /* If requested, generate keywords from the body of the article and
           patch them into the apparent value of the Keywords header field so
           that they make it into overview. */
        ...
        case HDR__XREF:
            ...
            break;
        case HDR__LINES:
            snprintf(data->LinesBuffer, sizeof(data->LinesBuffer), "%d",
                     data->Lines);
            p = data->LinesBuffer;
...
ARTpost(CHANNEL *cp)
{
     ...
     * logipaddr right now (it will be done afterwards and change
     * data->Feedsite
    ...
    if (!artclean && (!HDR_FOUND(HDR__PATH) || !HDR_FOUND(HDR__MESSAGE_ID))) {
        /* cp->Error is set since Path and Message-ID are required header
         * fields and one of them is not found during ARTclean(). We do not
         * remember the Message-ID of this article because another peer may
         * send it with a good Path header field. */
    ...
    if (!artclean) {
        ARTlog(data, ART_REJECT, cp->Error);
        /* If the article posting time has not been properly parsed,
         * data->Posted will be negative or zero. */
    ...
    if (Pathcluster.data != NULL && n == Pathcluster.used - 1
    ...
    for (j = 0; ME.Exclusions && ME.Exclusions[j]; j++) {
        if (ListHas((const char **) hops, (const char *) ME.Exclusions[j])) {
            snprintf(
                cp->Error, sizeof(cp->Error), "%d Unwanted site %s in path",
                ihave ? NNTP_FAIL_IHAVE_REJECT : NNTP_FAIL_TAKETHIS_REJECT,
    ...
    if (filterrc != NULL) {
        if (innconf->dontrejectfiltered) {
            Filtered = true;
            syslog(L_NOTICE,
                   "rejecting[python] %s %d %.200s (with dontrejectfiltered)",
                   HDR(HDR__MESSAGE_ID),
                   ihave ? NNTP_OK_IHAVE : NNTP_OK_TAKETHIS, filterrc);
        } else {
    ...
    if (filterrc) {
        if (innconf->dontrejectfiltered) {
            Filtered = true;
            syslog(L_NOTICE,
                   "rejecting[perl] %s %d %.200s (with dontrejectfiltered)",
                   HDR(HDR__MESSAGE_ID),
                   ihave ? NNTP_OK_IHAVE : NNTP_OK_TAKETHIS, filterrc);
        } else {
            snprintf(cp->Error, sizeof(cp->Error), "%d %.200s",
    ...
    if (HDR_FOUND(HDR__DISTRIBUTION)) {
        if (HDR(HDR__DISTRIBUTION)[0] == ',') {
            snprintf(
                cp->Error, sizeof(cp->Error), "%d Bogus distribution \"%s\"",
                ihave ? NNTP_FAIL_IHAVE_REJECT : NNTP_FAIL_TAKETHIS_REJECT,
    ...
    if (HDR_FOUND(HDR__CONTROL)) {
        ...
        *p = '\0';
        ...
        LikeNewgroup =
            (LikeNewgroup || strcasecmp(ControlWord, "checkgroups") == 0);
    ...
    for (GroupMissing = Accepted = false; (p = *groups) != NULL; groups++) {
        if ((ngp = NGfind(p)) == NULL) {
            ...
            if (LikeNewgroup && Approved) {
                /* Checkgroups/newgroup/rmgroup being sent to a group that
                 * doesn't exist.  Assume it is being sent to the group being
                 * created or removed (or to the admin group to which the
                 * checkgroups is posted), and send it to all sites that would
                ...
                for (ngpjunk = NGfind(ARTjnk), sp = Sites, i = nSites;
                     --i >= 0; sp++) {
                    if (sp->Name != NULL && sp->FeedTrash
                        && SITEwantsgroup(sp, *groups)) {
                        SITEmark(sp, ngpjunk);
                    }
                }
                NonExist = true;
                ...
                NonExist = true;
                /* Check if all the newsgroups listed in the Newsgroups header
                 * field of the article exist on the news server. */
    ...
    if (!Accepted || ngptr == GroupPointers) {
        if (!Accepted) {
            if (NoHistoryUpdate) {
                snprintf(
                    cp->Error, sizeof(cp->Error), "%d Can't post to \"%s\"",
                    ihave ? NNTP_FAIL_IHAVE_REJECT : NNTP_FAIL_TAKETHIS_REJECT,
            ...
            if (!innconf->wanttrash) {
                 ...
                 * innconf->remembertrash true, then you want all trash except
                 * that which you explicitly excluded in your active file. */
                if (!GroupMissing) {
                    if (innconf->remembertrash && (Mode == OMrunning)
                        && !NoHistoryUpdate
                        && !InndHisRemember(HDR(HDR__MESSAGE_ID),
    ...
    *ngptr = NULL;
    ...
    if ((innconf->enableoverview && !innconf->useoverchan) || NeedOverview) {
        ...
        if (innconf->enableoverview && !innconf->useoverchan) {
            if ((result =
                     OVadd(token, data->Overview.data, data->Overview.left,
    ...
    if (data->CRwithoutLF > 0 || data->LFwithoutCR > 0) {
        if (data->CRwithoutLF > 0 && data->LFwithoutCR == 0)
            ...
            snprintf(cp->Error, sizeof(cp->Error),
                     "Article accepted but includes CR without LF(%d) and LF "
                     "without CR(%d)",
