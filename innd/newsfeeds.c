...
SITEcopystrings(char **av)
{
    ...
    for (pp = av; *pp; pp++)
        continue;
    for (new = save = xmalloc((pp - av + 1) * sizeof(char *)), pp = av; *pp;
         pp++)
        *new ++ = xstrdup(*pp);
    *new = NULL;
    return save;
...
SITEaddvariable(char *line)
{
    ...
    if (*p != '=')
        return false;
    if (p - line > 32) {
        syslog(L_FATAL, "%s bad_newsfeed variable name '%s' too long", LogName,
               line + 1);
        return false;
    }
...
SITEreadfile(const bool ReadOnly)
{
    ...
    if (old_strings != NULL) {
        ...
        if (stat(SITEfeedspath, &Sb) >= 0 && Sb.st_ino == old_ino
            && Sb.st_size == old_size && Sb.st_mtime == old_mtime)
...
SITEsetlist(char **patlist, char *subbed, char *poison, bool *poisonEntry)
{
    ...
    while ((pat = *patlist++) != NULL) {
        ...
        if (*p == '\0') {
            ...
            for (p = subbed, u = poison, ngp = Groups, i = nGroups; --i >= 0;
                 ngp++, p++, u++)
    ...
    for (save = argv = CommaSplit(f3); (p = *argv++) != NULL;)
        ...
        case 'A':
            while (*++p)
                ...
                case 'C':
                    ...
                    break;
                case 'd':
                    sp->DistRequired = true;
                    break;
                case 'e':
                    sp->DontWantNonExist = true;
                    break;
                case 'f':
                    sp->DropFiltered = true;
                    break;
                case 'j':
                    sp->FeedTrash = true;
                    break;
                case 'o':
                    sp->NeedOverviewCreation = true;
                    break;
                case 'O':
                    sp->FeedwithoutOriginator = true;
                    break;
                case 'p':
                    sp->IgnorePath = true;
                    break;
        ...
        case 'B':
            if (*++p && isdigit((unsigned char) *p)) {
                ...
                if ((p = strchr(p, NF_SUBFIELD_SEP)) != NULL && *++p
        ...
        case 'Q':
            ...
            if (sscanf(p, "%u/%u", &hf->begin, &hf->mod) == 2) {
                hf->end = hf->begin;
            } else if (sscanf(p, "%u-%u/%u", &hf->begin, &hf->end, &hf->mod)
                       != 3) {
                free(hf);
                return "hash not in x/z or x-y/z format for Q param in field "
                       "3";
            }
        ...
        case 'W':
            for (i = 0; *++p && i < FEED_MAXFLAGS;) {
                ...
                case FEED_HASH:        /* Hash */
                case FEED_HDR_DISTRIB: /* Distribution header field */
                case FEED_STOREDGROUP: /* Stored newsgroup */
                    NeedStoredGroup = true;
                    break;
                case FEED_HDR_NEWSGROUP: /* Newsgroup header field */
...
SITEparsefile(bool StartSite)
{
    ...
    for (sp = Sites, errors = 0, setuperrors = 0, i = 0; i < nSites; i++) {
        ...
        if (p[0] == 'M' && p[1] == 'E'
            ...
            if (ME.Prev == NOSITE) {
                syslog(L_FATAL,
                       "bad_newsfeeds. Must have exactly one ME entry");
                errors++;
            } else if ((error = SITEparseone(p, &ME, subbed, poison))
                       != NULL) {
