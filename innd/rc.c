...
RCCommaSplit(char *text)
{
    ...
    *q = '\0';
    free(text);
    for (text = r, av = save = xmalloc(i * sizeof(char *)), *av++ = p = text;
         *p;)
        if (*p == ',') {
            *p++ = '\0';
            *av++ = p;
...
RCreader(CHANNEL *cp)
{
    ...
    if (j == chanlimit) {
        syslog(L_ERROR, "%s internal RCreader wrong channel 0x%p", LogName,
               (void *) cp);
        return;
    }
    ...
    if (RemoteTimer != 0) {
        ...
        while (count--) {
            ...
            if (network_sockaddr_equal(
                    (struct sockaddr *) &remotetable[i].Address,
        ...
        if (remotecount == RemoteTotal) {
            ...
            reject_val = NNTP_FAIL_TERMINATING;
            xasprintf(&reject_message,
                      "%d Connection rejected, you're making too"
            ...
            if (new->MaxCnx > 0 && new->HoldTime == 0) {
                ...
                if ((new->ActiveCnx > new->MaxCnx) && (new->fd > 0)) {
                    snprintf(buff, sizeof(buff),
                             "You are limited to %d connection%s", new->MaxCnx,
                             (new->MaxCnx != 1) ? "s" : "");
                    NCwriteshutdown(new, buff);
                    syslog(L_NOTICE, "too many connections from %s",
                           rp->Label);
                } else {
...
RCreaddata(int *num, FILE *F, bool *toolong)
{
    ...
    *toolong = false;
    if (*RCbuff == '\0') {
        if (feof(F))
            return (NULL);
        if (fgets(RCbuff, sizeof RCbuff, F) == NULL)
            return (NULL);
        (*num)++;
    ...
    while ((word = RCreaddata(&linecount, F, &toolong)) != NULL) {
        ...
        if (!strncmp(word, GROUP, sizeof GROUP)) {
            ...
            if (groupcount == 1) {
                ...
                groups =
                    xrealloc(groups, (groupcount + 4) * sizeof(REMOTEHOST));
                maxgroup += 5;
            ...
            group_params->Skip = groupcount > 1 ? groups[groupcount - 2].Skip
                                                : default_params.Skip;
            group_params->Ignore = groupcount > 1
                                       ? groups[groupcount - 2].Ignore
            ...
            group_params->Password = groupcount > 1
                                         ? groups[groupcount - 2].Password
                                         : default_params.Password;
            group_params->Identd = groupcount > 1
                                       ? groups[groupcount - 2].Identd
                                       : default_params.Identd;
            group_params->MaxCnx = groupcount > 1
                                       ? groups[groupcount - 2].MaxCnx
                                       : default_params.MaxCnx;
            group_params->HoldTime = groupcount > 1
                                         ? groups[groupcount - 2].HoldTime
                                         : default_params.HoldTime;
        ...
        if (!strncmp(word, PEER, sizeof PEER)) {
            ...
            if (peer_params.Label != NULL) {
                ...
                syslog(L_ERROR, PEER_IN_PEER, LogName, filename, linecount);
                break;
            }
            if ((word = RCreaddata(&linecount, F, &toolong)) == NULL) {
            ...
            peer_params.Skip =
                groupcount > 0 ? group_params->Skip : default_params.Skip;
            peer_params.Ignore =
            ...
            peer_params.Password = groupcount > 0 ? group_params->Password
                                                  : default_params.Password;
            peer_params.Identd =
                groupcount > 0 ? group_params->Identd : default_params.Identd;
            peer_params.MaxCnx =
                groupcount > 0 ? group_params->MaxCnx : default_params.MaxCnx;
            peer_params.HoldTime = groupcount > 0 ? group_params->HoldTime
                                                  : default_params.HoldTime;
        ...
        if (!strncmp(word, "}", 1)) {
            ...
            if (peer_params.Label != NULL) {
                ...
                for (r = q = RCCommaSplit(xstrdup(peer_params.Name));
                     *q != NULL; q++) {
                    ...
                    *list = xrealloc(*list, *count * sizeof(REMOTEHOST));
                    ...
                    if ((gai_ret = getaddrinfo(*q, NULL, &hints, &res0))
                        != 0) {
                        syslog(L_ERROR, "%s cant getaddrinfo %s %s", LogName,
                               *q, gai_strerror(gai_ret));
                        /* decrement *count, since we never got to add this
                         * record. */
                        (*count)--;
                        continue;
                    }
                    ...
                    *list = xrealloc(*list, *count * sizeof(REMOTEHOST));
                    ...
                    /* Add all hosts */
                    for (res = res0; res != NULL; res = res->ai_next) {
                        (void) memcpy(&rp->Address, res->ai_addr,
                                      res->ai_addrlen);
                        rp->Name = xstrdup(*q);
                        ...
                        rp->Patterns =
                            peer_params.Pattern != NULL
                                ? RCCommaSplit(xstrdup(peer_params.Pattern))
                                : NULL;
        ...
        if (toolong)
            ...
            syslog(L_ERROR, "%s Unknown value line %d: %s", LogName, linecount,
                   filename);
        free(word);
        break;
    }
...
RCcanpost(CHANNEL *cp, char *group)
{
    ...
    for (rp = RCpeerlist, i = RCnpeerlist; --i >= 0; rp++) {
        ...
        for (match = 0, argv = rp->Patterns; (pat = *argv++) != NULL;) {
            subvalue = (*pat != SUB_NEGATE) && (*pat != SUB_POISON) ? 0 : *pat;
            if (subvalue)
                pat++;
