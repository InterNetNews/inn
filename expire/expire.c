...
**  written between our read and the eventual unlink.  consuming=
**  false (dry-run, tracing, or alternate-output expire) skips the
**  rename and reads the live file directly: no consume happens, so
...
**  all reads failed); that is not an error.  Expire just falls back
**  to the per-article SMretrieve check.  *out_expireover_path and
**  *out_cancels_snapshot are set to the file paths (caller frees and
**  unlinks after a successful run); both are NULL when consuming is
**  false.
*/
static struct hash *
