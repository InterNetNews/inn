...
PushIOvRateLimited(void)
{
    ...
    while (queued_iov) {
        ...
        sentiov = 0;
        for (i = 0; (i < queued_iov) && (bytesfound < MaxBytesPerSecond);
             i++) {
        ...
        memmove(iov, &iov[sentiov],
                (queued_iov - sentiov) * sizeof(struct iovec));
        queued_iov -= sentiov;
    }
}
...
SendIOv(const char *p, int len)
{
    ...
    if (queued_iov) {
        q = (char *) iov[queued_iov - 1].iov_base
            + iov[queued_iov - 1].iov_len;
        if (p == q) {
...
SendIOb(const char *p, int len)
{
    ...
    while (len > 0) {
        tocopy =
            (len > (BIG_BUFFER - highwater)) ? (BIG_BUFFER - highwater) : len;
...
**
**  Freshness is keyed on both mtime and size: mtime alone has
**  1-second granularity, so a rename-and-recreate within the same
...
ARTinstorebytoken(TOKEN token)
{
    ...
    if (innconf->expiretombstone && PERMaccessconf->nnrpdcheckart
        && !SMprobe(SELFEXPIRE, &token, NULL) && nnrpd_tombstone_refresh()) {
...
ARTsendmmap(SENDTYPE what)
{
    ...
    if (what == STarticle) {
        ...
        for (q = p = ARThandle->data; p < (ARThandle->data + ARThandle->len);
             p++) {
            if (*p == '\r')
                continue;
            if (*p == '\n') {
    ...
    if (VirtualPathlen > 0 && (what != STbody)) {
        ...
        if (path == NULL) {
            ...
            xref =
                wire_findheader(ARThandle->data, ARThandle->len, "Xref", true);
            if (xref == NULL) {
                SendIOv(".\r\n", 3);
                ARTgetsize += 3;
                PushIOv();
            ...
            if (strncasecmp(p, header, headerlen) == 0 && p[headerlen] == ':'
                ...
                if (pathheader && (VirtualPathlen > 0)) {
                    ...
                    if (endofpath == NULL)
                        return NULL;
                    for (s = p, prevchar = '\0';
                         s + VirtualPathlen + 1 < endofpath; prevchar = *s++) {
                        if ((prevchar != '\0' && prevchar != '!')
                            || *s != *VirtualPath
                            || strncasecmp(s, VirtualPath, VirtualPathlen - 1)
                                   != 0)
                            continue;
                        if (*(s + VirtualPathlen - 1) != '\0'
                            && *(s + VirtualPathlen - 1) != '!')
...
CMDover(int ac, char *av[])
{
    ...
    if ((handle = (void *) OVopensearch(GRPcur, range.Low, range.High))
        == NULL) {
    ...
    while (OVsearch(handle, &artnum, &data, &len, &token, NULL)) {
        ...
        if (len == 0
            || (PERMaccessconf->nnrpdcheckart && !ARTinstorebytoken(token))) {
            if (PERMaccessconf->nnrpdoverstats) {
                OVERmiss++;
                gettimeofday(&stv, NULL);
            }
...
CMDpat(int ac, char *av[])
{
    ...
    do {
        ...
        if (mid) {
            ...
            Reply(
                "%d Header information for %s follows (from the article)\r\n",
        ...
        if (Overview < 0 || IsBytes || IsLines) {
            ...
            if (HasNotReplied) {
                if (hdr) {
                    ...
                    Reply("%d No header information for %s follows (from "
                          "articles)\r\n",
                          NNTP_OK_HEAD, av[1]);
                    Printf(".\r\n");
                }
        ...
        while (OVsearch(handle, &artnum, &data, &len, &token, NULL)) {
            if (len == 0
                || (PERMaccessconf->nnrpdcheckart
                    && !ARTinstorebytoken(token)))
                continue;
            if (HasNotReplied) {
                Reply("%d Header or metadata information for %s follows (from "
                      "overview)\r\n",
                      hdr ? NNTP_OK_HDR : NNTP_OK_HEAD, av[1]);
                HasNotReplied = false;
            }
            ...
            if (p != NULL) {
                if (PERMaccessconf->virtualhost && Overview == overhdr_xref) {
                    p = vhost_xref(p);
                    if (p == NULL) {
                        if (hdr) {
                            snprintf(buff, sizeof(buff), "%lu \r\n", artnum);
        ...
        if (HasNotReplied) {
            if (hdr) {
                ...
                Reply("%d No header or metadata information for %s follows "
                      "(from overview)\r\n",
                      NNTP_OK_HEAD, av[1]);
                Printf(".\r\n");
            }
