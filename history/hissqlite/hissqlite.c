...
**  paused).  SQLite's WAL write lock and busy_timeout serialize the two without
**  a broker server process (unlike ovsqlite): this works because writes
**  autocommit, so the lock is held only per statement and the periodic, light
**  expire does not contend with innd for long.  See hissqlite(5).
**
          ...
          size_t size, const char *what)
{
    if (sqlite3_column_bytes(stmt, col) != (int) size) {
        ...
        if (h->history != NULL)
            his_seterror(h->history, concat("hissqlite: ", what, (char *) NULL));
        return false;
    }
...
hissqlite_wal_passive(struct hissqlite *h)
{
    ...
    rc = sqlite3_wal_checkpoint_v2(h->db, NULL, SQLITE_CHECKPOINT_PASSIVE, &log,
                                  &ckpt);
    if (rc != SQLITE_OK) {
    ...
    if (log == ckpt)
        h->wal_pages = 0; /* Fully flushed; the WAL resets on the next write. */
...
**  database but must NOT re-run the DDL (or re-stamp the version) on an existing
**  one -- otherwise opening an existing DB with HIS_CREAT fails on "table hist
**  already exists" and would mask a version mismatch.
*/
static bool
...
hissqlite_doopen(struct hissqlite *h)
{
    ...
    if ((h->flags & HIS_CREAT) && !hissqlite_initialized(h)) {
        ...
        if (sqlite_helper_init(&hissqlite_init_helper,
                               (sqlite3_stmt **) &init, h->db, 0, &errmsg)
            != SQLITE_OK) {
...
hissqlite_sync(void *history)
{
    ...
    /* HISsync is deliberately NOT a durability barrier, matching dbz: hisv6_sync
       does fflush(3) + msync(MS_ASYNC), neither of which waits for the disk, so
       dbz history is only kernel-buffered after a sync (survives an innd crash,
       lost on power loss).  Under synchronous=NORMAL each autocommit already
       write(2)s the WAL into the kernel page cache, so hissqlite sits
       continuously in that same state: there is nothing application-buffered
       for HISsync to flush.  (Per-commit durability would be synchronous=FULL, a
       possible future knob, not an fsync here.)  innd calls HISsync on a timer,
       so it is simply the convenient place to keep the WAL bounded: once it has
       grown past the threshold, do a non-blocking PASSIVE checkpoint. */
...
hissqlite_close(void *history)
{
    ...
    /* A deferred handle (HISopen(NULL) whose HISCTLS_PATH was never set or
       ...
       close; just free it.  Skipping this would call sqlite3_wal_checkpoint_v2()
       on a NULL connection, which is not a no-op like sqlite3_close_v2(NULL). */
    if (h->db == NULL) {
        free(h->path);
        free(h);
        return true;
                 ...
                 bool (*decide)(void *, time_t, time_t, time_t, struct token *))
{
     ...
     * retention is driven by expireover plus storage existence and the tombstone
     * log rather than expire.ctl: this backend is decision-agnostic and only
     * applies the verdict in place.
     *
     ...
     * UPDATE/DELETE so the lock is held one statement at a time.  Pass 1 streams
     * the scan in hash-keyset pages: each page reads under its own short read
     * snapshot which is reset before the page is applied, so a long expire never
     * pins the WAL (which would block innd's checkpoints) and RAM is bounded to
     * one page rather than the whole change set.
     */
    ...
    /* Pass 2: delete remembered entries past the /remember/ threshold, in
       bounded chunks (the statement deletes up to a LIMIT per step).  Each step
       autocommits, so the write lock is released between chunks and innd's
       accepts interleave; loop until a chunk deletes nothing. */
    if (writing && ok) {
