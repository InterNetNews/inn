...
**
**  Reads the source history (default method "hisv6") via HISwalk and bulk-loads
**  every entry into a freshly created hissqlite database keyed by hash.  This
**  is a faithful migration (the makedbz analog): it preserves exact
**  timestamps AND remembered (token-less) entries, which a from-spool rebuild
**  (makehistory) cannot reconstruct.
**
**  Insertion is by HASH, not Message-ID: the history text stores only the MD5,
**  and the original Message-ID is unrecoverable, so HISwrite (which re-hashes a
**  Message-ID) cannot be used.  We therefore create the canonical schema via the
**  hissqlite backend, then bulk-insert by hash through a raw SQLite connection.
**
