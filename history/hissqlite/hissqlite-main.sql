...
-- per-connection page cache is affordable; mmap lets reads come from the shared
-- OS page cache and keeps the random-MD5 B-tree hot.
...
-- an fsync, so there is no fsync cost to amortise; batching would only hold the
-- write lock across the batch and starve the other writer (expire) in the
...
-- HIScheck: existence only (real OR remembered both count -> refuse re-offers).
-- his.c keeps an in-memory existence cache in front of this; this is the
...
-- existing row rather than failing: a duplicate Message-ID during a makehistory
-- rebuild must warn and continue, not abort the whole rebuild, as in hisv6
-- (hissqlite.c detects the no-op via sqlite3_changes()).
-- .write
...
-- step (LIMIT) so each autocommit holds the write lock only briefly; the caller
-- loops until a step deletes nothing.
-- .expire_remembered
