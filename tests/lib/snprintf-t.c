...
static void
ok_format(int n, bool truncate, const char *expected, int count,
          const char *format, ...)
{
    char buf[128];
    int result;
...
main(void)
{
    ...
    ok_format(6, true, "abcd", 4, "%s", "abcd");
    ok_format(7, true, "20", 2, "%d", 20);
    ok_format(8, true, "Test ab", 7, "Test %.2s", "abcd");
    ok_format(9, true, "a", 1, "%c", 'a');
    ok_format(10, true, "", 0, "");
    ok_format(11, true, "abcdefghijklmnopqrstuvwxyz01234", 36, "%s", string);
    ok_format(12, true, "abcdefghij", 10, "%.10s", string);
    ok_format(13, true, "  abcdefghij", 12, "%12.10s", string);
    ok_format(14, true, "    abcdefghijklmnopqrstuvwxyz0", 40, "%40s",
              string);
    ok_format(15, true, "abcdefghij    ", 14, "%-14.10s", string);
    ok_format(16, true, "              abcdefghijklmnopq", 50, "%50s",
              string);
    ok_format(17, true, "%abcd%", 6, "%%%0s%%", "abcd");
    ok_format(18, true, "", 0, "%.0s", string);
    ok_format(19, true, "abcdefghijklmnopqrstuvwxyz  444", 32, "%.26s  %d",
              string, 4444);
    ok_format(20, true, "abcdefghijklmnopqrstuvwxyz  -2.", 32, "%.26s  %.1f",
              string, -2.5);

    ok_format(21, true, "abcdefghij4444", 14, "%.10s%n%d", string, &count,
              4444);
    ok(22, count == 10);
    ok_format(23, true, "abcdefghijklmnopqrstuvwxyz01234", 36, "%ln%s%n",
              &count, string, &lcount);
    ok(24, count == 0);
    ok(25, lcount == 31);
