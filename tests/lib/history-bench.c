...
print_csv_header(void)
{
    printf("\nmethod,entries,random_lookups,bytes,write_s,write_per_s,sync_s,"
           "close_s,open_ro_s,check_s,check_per_s,lookup_s,lookup_per_s,"
           "missing_s,missing_per_s,path\n");
}
...
print_csv(const struct bench_result *result)
{
    if (result->skipped)
        return;
    printf(
        "%s,%llu,%llu,%llu,%.6f,%.2f,%.6f,%.6f,%.6f,%.6f,%.2f,%.6f,"
        "%.2f,%.6f,%.2f,%s\n",
        ...
        rate(result->entries, result->check_seconds), result->lookup_seconds,
        rate(result->random_lookups, result->lookup_seconds),
        result->missing_seconds,
