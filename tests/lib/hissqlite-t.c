...
**  walk, the two-horizon expire (real->remembered transition + remember-delete),
**  sync, close, and persistence across reopen.
**
...
static bool
decide_keep_all(void *cookie UNUSED, time_t arrived UNUSED, time_t posted UNUSED,
                time_t expires UNUSED, TOKEN *token UNUSED)
{
    return true;
}
...
main(void)
{
    ...
    ok(15,
       HISexpire(h, NULL, NULL, true, NULL, (time_t) (BASE - 1), decide_drop_old));
    ...
    ok(17, HISexpire(h, NULL, NULL, true, NULL, BASE + 1000000, decide_keep_all));
    /* original remembered (#N_TOKEN+5) gone; surviving token (#90) stays */
    {
        char *m = make_msgid(N_TOKEN + 5);
        bool rem_gone = !HIScheck(h, m);
        bool tok_stays = present_with_token(h, 90, &has_token, NULL) && has_token;
       ...
       would drop stays a real token. */
    {
        ...
        expire_cutoff = BASE + 100; /* decide_drop_old would drop arrived=BASE */
        if (eh != NULL)
            HISexpire(eh, NULL, NULL, false, NULL, BASE + 1000000,
                      decide_drop_old);
        memset(&got, 0, sizeof(got));
        kept = eh != NULL
               && HISlookup(eh, "<dry@test>", NULL, NULL, NULL, &got)
